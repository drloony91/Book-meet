...
## Flagged group-chat UI slices 1–2

`tests/group-chat-ui-contract.test.mjs` checks the distinct group route, closed client gate and abort-capable hook, then exercises resettable demo lifecycle, candidate search, avatar, paged history, message/read/search/reaction and poll flow with the same `BOOK_MEET_GROUP_CHATS_ENABLED` projection. `tests/group-conversations-http-mysql.mjs` covers transactional production roles, moderation, polls, current block/age visibility, avatar input and newest/older page cursors inside `pnpm verify:db`. `tests/browser/group-chats.spec.ts` enables the otherwise closed flag only for the isolated demo server and runs the create/avatar/candidate/message/search/emoji/sticker/poll/settings/delete lifecycle in both desktop and mobile projects.

For flagged stage 6B, `tests/reading-sessions.test.mjs` checks strict manual input, lease flag and local-midnight slices. `tests/reading-statistics.test.mjs` checks once-per-book completion and exact-second aggregation with an unflushed timer. `tests/reading-presence.test.mjs` checks fail-closed audience decisions. Guarded disposable MySQL runs `tests/reading-sessions-http-mysql.mjs` for ownership, one active timer, lease expiry, manual CRUD, completed-cycle attribution, statistics recalculation, midnight split and library unlink; `tests/reading-presence-http-mysql.mjs` covers current audience, block, age, book and lease boundaries. The reading-session flag remains closed by default. `tests/reading-demo-http.test.mjs` checks flagged demo session, presence and statistics parity; `tests/browser/reading-sessions.spec.ts` covers focused timer restoration and manual CRUD on desktop/mobile. The segmented statistics UI has a separate browser acceptance test.

For Stage 6C, `tests/marketplace-schema.test.mjs` checks the closed bootstrap flag and additive migrations 056–057. `tests/marketplace-listings.test.mjs` checks listing input validation, fail-closed routes, age and ownership before image writes, blocked-seller filtering and pagination, including the UTC adult cutoff. Disposable-MySQL `tests/marketplace-listings-http-mysql.mjs` covers creation, duplicates, current seller age, owner, block, filters, status and soft removal; `tests/marketplace-images-http-mysql.mjs` covers signature checks, transaction cleanup, replacement, private serving, current seller age, minor/block denial and retained files after soft removal; `tests/marketplace-conversations-http-mysql.mjs` covers unique buyer/listing dialogue, a concurrent three-message cap, block and current participant-age enforcement, per-viewer hiding and neutral/hidden post-removal history; `tests/marketplace-moderation-http-mysql.mjs` covers report projections, seller restriction/lift and audited removal. `tests/browser/marketplace.spec.ts` renders the flagged listing/conversation route, separate listing/seller reporting, owner create/reserve/remove and minor-access projection on desktop/mobile with mocked marketplace responses; `tests/browser/marketplace-moderation.spec.ts` checks the admin evidence and action UI with mocked marketplace responses. Real-API visual acceptance is still outstanding. The flag stays closed by default and legal confirmation is required before release.
