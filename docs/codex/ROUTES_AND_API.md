...
| Books/library | `GET /books/catalog?q=` (trimmed, normalized catalogue; empty `q` returns the full permitted catalogue, non-empty `q` is bounded to 100), `/books`; `POST /books`, `PATCH`/`DELETE /books/:id`; `POST /books/preview`; admin import preview/resolve | catalog viewer; writes owner/session; `DELETE /books/:id` removes only the caller's `user_books` relation; import admin | `server/api.js`, `server/data.js`, `material-input.js`, image storage |
