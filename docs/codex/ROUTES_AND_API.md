...
| Mobile-only workflows | `/search`, `/create|edit/{review,publication,event,occasion,book,news}[/:id]`, `/edit/book-status/:id`, `/profile/{followers,following,friends,incoming,outgoing}` | `MobileGlobalSearchPage.tsx`, mobile forms and `routes.ts` | mobile media query; mutation requires session/ownership |
