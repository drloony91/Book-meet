...
| --- | --- | --- | --- |
| Public shell | `/`, `/users`, `/books`, `/publishing`, `/events`, `/reviews`, `/meet`, `/blog`, `/communities`, `/partners` | `GuestExperience.tsx`, `AppLayout.tsx`, `useBookMeetController.tsx` | catalog can be guest-safe; signed-in actions require session |
...
| --- | --- | --- | --- |
| Health/public/auth | `GET /health`, `/auth/providers`, `/auth/legal-documents`, `/public/catalog`; `/auth/login`, `/auth/register`, Google callback/credential, email verify/reset, logout, deleted restore/new | health/public mostly guest; mutations create or require session as flow dictates | `server/api.js`, `server/security.js`, `server/modules/compliance.js`, `server/modules/public-catalog.js` |
...
| Account/profile | `PUT /users/me/state`, `PATCH /users/me/home-view`, `/users/me/profile-complete`, `DELETE /users/me/profile`; `GET/PATCH/DELETE /users/me/telegram*`; linked profile create/attach/google/switch/delete | current user; destructive delete/merge has explicit session checks | `server/api.js`, `server/modules/account-tokens.js`, `compliance.js` |
...
| Books/library | `GET /books/catalog`, `/books`; `POST /books`, `DELETE /books/:id`; `POST /books/preview`; admin import preview/resolve | catalog viewer; writes owner/session; import admin | `server/api.js`, `server/data.js`, `material-input.js`, image storage |
| Materials | `DELETE /materials/:kind/:id`; admin patch/delete `/admin/materials/:kind/:id` | owner for own deletion; admin for moderation | `server/api.js`, `content-security.js`, `compliance.js` |
