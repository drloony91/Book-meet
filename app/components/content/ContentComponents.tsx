...
import { MentionRichText, MentionText } from "./MentionText";
import { CleanRepostCard, HideUserAction, RepostAction } from "./SocialMaterialActions";
...
export type EventFormValue = { title: string; summary: string; description: string; isAdult: boolean; date: string; time: string; city: string; cityId?: number; address: string; mapUrl: string; detailsUrl: string; relatedToBook: boolean; linkedBookId?: number; linkedBookIds: number[]; mentions?: MentionRef[] };
export const emptyEvent: EventFormValue = { title: "", summary: "", description: "", isAdult: false, date: "", time: "", city: "", address: "", mapUrl: "", detailsUrl: "", relatedToBook: false, linkedBookIds: [], mentions: [] };
...
export function EventForm({ initial, catalog = [], onCreateBook = () => undefined, onCancel, onSave, submitLabel }: { initial?: BookEvent; catalog?: (LibraryBook | AuthorBook)[]; onCreateBook?: () => void; onCancel: () => void; onSave: (value: EventFormValue) => Promise<void>; submitLabel?: string }) {
  ...
  return <form className="event-form" onSubmit={async (event) => { event.preventDefault(); setSaving(true); try { await onSave(value); } finally { setSaving(false); } }}>
    ...
    <label>{t("event.title")}<MentionTextarea required rows={2} maxLength={200} {...field("title")} mentionUserIds={(value.mentions ?? []).map((item) => item.userId)} onMentionUserIdsChange={() => undefined} mentionRefs={value.mentions} onMentionRefsChange={(mentions) => setValue((current) => ({ ...current, mentions }))} /></label>
    <label>{t("event.summary")}<MentionTextarea required rows={3} maxLength={1200} {...field("summary")} mentionUserIds={(value.mentions ?? []).map((item) => item.userId)} onMentionUserIdsChange={() => undefined} mentionRefs={value.mentions} onMentionRefsChange={(mentions) => setValue((current) => ({ ...current, mentions }))} /></label>
    <label>{t("event.details")}<MentionTextarea required rows={6} {...field("description")} mentionUserIds={(value.mentions ?? []).map((item) => item.userId)} onMentionUserIdsChange={() => undefined} mentionRefs={value.mentions} onMentionRefsChange={(mentions) => setValue((current) => ({ ...current, mentions }))} /></label>
...
type MaterialActionProps = { likesCount?: number; commentsCount?: number; savesCount?: number; liked?: boolean; saved?: boolean; onToggleLike?: () => void; onOpenComments?: () => void; onToggleSave?: () => void; shareAttachment?: ChatAttachment; repostAction?: React.ReactNode };
...
export function MaterialActionBar({ likesCount = 0, commentsCount = 0, savesCount = 0, liked = false, saved = false, onToggleLike, onOpenComments, onToggleSave, shareAttachment, repostAction }: MaterialActionProps) {
  const { t } = useI18n();
...
export function MaterialEngagement({ kind, materialId, ownerId, currentUser, users, onOpenUser, shareAttachment }: { kind: EngagementKind; materialId: number; ownerId: number; currentUser?: DemoUser; users: DemoUser[]; onOpenUser?: (userId: number) => void; shareAttachment?: ChatAttachment }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      apiFetch("/api/material-stats", { credentials: "same-origin", cache: "no-store" }).then((response) => response.json() as Promise<{ commentCounts?: Record<string, number>; saveCounts?: Record<string, number>; repostCounts?: Record<string, number> }>),
    ]).then(([reactions, saveState, materialStats]) => {
      if (!active) return;
      setLikedUserIds(reactions.userIds ?? []);
      setCommentCount(materialStats.commentCounts?.[`${kind}-${materialId}`] ?? 0);
      setSaved(Boolean(saveState.saved));
      setSavesCount(materialStats.saveCounts?.[`${kind}-${materialId}`] ?? 0);
      setRepostCount(materialStats.repostCounts?.[`${kind}-${materialId}`] ?? 0);
  ...
  return <section className="material-engagement"><MaterialActionBar likesCount={likedUserIds.length} commentsCount={commentCount} savesCount={savesCount} liked={liked} saved={saved} repostAction={currentUser ? <RepostAction kind={kind} materialId={materialId} repostCount={repostCount} /> : undefined} shareAttachment={shareAttachment} onToggleLike={() => void toggleLike()} onOpenComments={() => document.querySelector<HTMLTextAreaElement>(`.material-engagement textarea`)?.focus()} onToggleSave={() => void toggleSave()} /><SocialComments kind={kind} materialId={materialId} currentUser={currentUser} users={users} onOpenUser={onOpenUser} onCountChange={setCommentCount} /></section>;
}
...
export function EventModal({ item, users = [], catalog: canonicalCatalog = [], currentUserId, currentUser, onClose, onOpenBook, onOpenUser, onEdit, onDelete, onReport }: { item: BookEvent; users?: DemoUser[]; catalog?: (LibraryBook | AuthorBook)[]; currentUserId?: number; currentUser?: DemoUser; onClose: () => void; onOpenBook?: (bookId?: number) => void; onOpenUser?: (userId: number) => void; onEdit?: () => void; onDelete?: () => void; onReport?: () => void }) {
  ...
  return <div className="modal-backdrop entity-page-backdrop" onMouseDown={routedClose}><section className="event-modal" onMouseDown={(event) => event.stopPropagation()}>
    <ModalIconActions leading={viewer && viewer.id !== item.creatorId ? <HideUserAction userId={item.creatorId} icon onHidden={routedClose} /> : undefined} onEdit={onEdit} onDelete={onDelete} onReport={onReport} onClose={routedClose} />
    ...
    <span className="section-subtitle">{t("material.typeEvent")}{item.isAdult ? " · 18+" : ""} · <span data-i18n-skip>{item.city}</span></span><h2 data-i18n-skip><MentionText text={item.title} mentions={item.mentions} onOpenUser={onOpenUser} /></h2>
    ...
    <div className="event-modal-meta"><strong>{formatDate(`${item.date}T00:00:00`, { day: "numeric", month: "long", year: "numeric" })} · {item.time}</strong><span data-i18n-skip>{item.address}</span></div><p data-i18n-skip><MentionText text={item.description} mentions={item.mentions} onOpenUser={onOpenUser} /></p>
...
export function OccasionForm({ initial, catalog = [], onCancel, onSave, submitLabel }: { initial?: Occasion; catalog?: (LibraryBook | AuthorBook)[]; onCancel: () => void; onSave: (value: typeof emptyOccasion) => Promise<void>; submitLabel?: string }) {
  ...
  return <form className="occasion-form" onSubmit={async (event) => { event.preventDefault(); if (!value.type || incompleteTime || targetMissing || locationMissing || bookMissing) return; setSaving(true); try { await onSave({ ...value, targetCities: value.type === "invite" ? [value.meetingCity] : value.targetCities }); } finally { setSaving(false); } }}>
    ...
    {value.type && <>
      ...
      <label>{t(textKeys.primary)}<MentionTextarea required rows={5} value={value.primaryText} onChange={(event) => setValue({ ...value, primaryText: event.target.value })} placeholder={value.type === "invite" ? t("occasion.offerPlaceholder") : undefined} mentionUserIds={(value.mentions ?? []).map((item) => item.userId)} onMentionUserIdsChange={() => undefined} mentionRefs={value.mentions} onMentionRefsChange={(mentions) => setValue((current) => ({ ...current, mentions }))} /></label>
      <fieldset className="occasion-audience-box"><legend>{t("occasion.who")}</legend><label>{t(textKeys.audience)}<MentionTextarea required rows={4} value={value.audienceText} onChange={(event) => setValue({ ...value, audienceText: event.target.value })} mentionUserIds={(value.mentions ?? []).map((item) => item.userId)} onMentionUserIdsChange={() => undefined} mentionRefs={value.mentions} onMentionRefsChange={(mentions) => setValue((current) => ({ ...current, mentions }))} /></label><div className="occasion-audience-grid"><label>{t("occasion.gender")}<CustomSelect ariaLabel={t("occasion.gender")} value={value.targetGender} onChange={(targetGender) => setValue({ ...value, targetGender })} options={["Все", "Мужской", "Женский"].map((item) => ({ value: item as Occasion["targetGender"], label: domainLabel(item) }))} /></label><label>{t("occasion.profileType")}<CustomSelect ariaLabel={t("occasion.profileType")} value={value.targetProfileType} onChange={(targetProfileType) => setValue({ ...value, targetProfileType })} options={["Все", "Читатель", "Писатель", "Блогер"].map((item) => ({ value: item as Occasion["targetProfileType"], label: domainLabel(item) }))} /></label></div></fieldset>
...
export function OccasionModal({ item, currentUser, users = [], onClose, onOpenUser, onOpenBook, onEdit, onDelete, onReport }: { item: Occasion; currentUser?: DemoUser; users?: DemoUser[]; onClose: () => void; onOpenUser?: (id: number) => void; onOpenBook?: (id: number) => void; onEdit?: () => void; onDelete?: () => void; onReport?: () => void }) {
  ...
  return <div className="modal-backdrop entity-page-backdrop" onMouseDown={routedClose}><section className="event-modal occasion-modal" onMouseDown={(event) => event.stopPropagation()}>
    <ModalIconActions leading={currentUser && currentUser.id !== item.creatorId ? <HideUserAction userId={item.creatorId} icon onHidden={routedClose} /> : undefined} onEdit={onEdit} onDelete={onDelete} onReport={onReport} onClose={routedClose} />
    ...
    <p className="occasion-audience-row"><strong>{t(labels.primary)}:</strong> <span data-i18n-skip><MentionText text={item.primaryText} mentions={item.mentions} onOpenUser={onOpenUser} /></span></p>
    <p className="occasion-audience-row"><strong>{t(labels.audience)}:</strong> <span data-i18n-skip><MentionText text={item.audienceText} mentions={item.mentions} onOpenUser={onOpenUser} /></span></p>
...
export function PublisherNewsModal({ item, owner, currentUser, users = [], catalog = [], onClose, onOpenUser, onEdit, onDelete, onReport, routed = true }: { item: PublisherNews; owner?: DemoUser; currentUser?: DemoUser; users?: DemoUser[]; catalog?: (LibraryBook | AuthorBook)[]; onClose: () => void; onOpenUser: (id: number) => void; onEdit?: () => void; onDelete?: () => void; onReport?: () => void; routed?: boolean }) {
  ...
  if (!routedPopup.active) return null;
  return <div className="modal-backdrop entity-page-backdrop" onMouseDown={routedPopup.close}><section className="reading-modal publisher-news-modal" onMouseDown={(event) => event.stopPropagation()}>
    <ModalIconActions leading={currentUser && currentUser.id !== item.ownerId ? <HideUserAction userId={item.ownerId} icon onHidden={routedPopup.close} /> : undefined} onEdit={onEdit} onDelete={onDelete} onReport={onReport} onClose={routedPopup.close} />
    <header className="material-author-row"><div data-i18n-skip className={`author-dot ${owner?.avatarUrl ? "has-photo" : ""}`} style={owner?.avatarUrl ? { backgroundImage: `url(${owner.avatarUrl})` } : undefined}>{!owner?.avatarUrl && (owner?.initials ?? "BM")}</div><div><button className="inline-user-link" data-i18n-skip type="button" onClick={() => onOpenUser(item.ownerId)}>{owner?.profile.name ?? domainLabel(community ? "Сообщество" : "Издатель")}{owner?.username ? ` (@${owner.username})` : ""}</button><small>{t(community ? "material.typeCommunityNews" : "material.typePublisherNews")} · <span data-i18n-skip>{item.createdAt}</span></small></div></header><h2 data-i18n-skip><MentionText text={item.title} mentions={item.mentions} onOpenUser={onOpenUser} /></h2><p className="reading-preview" data-i18n-skip><MentionText text={item.previewText} mentions={item.mentions} onOpenUser={onOpenUser} /></p><MentionRichText className="reading-text rich-reading-text" html={renderRichHtml(item.bodyHtml, catalog.length ? catalog : catalogFromUsers(users), { inlineImageAlt: t("editor.inlineImage"), noAnnotation: t("book.noAnnotation") })} mentions={item.mentions} onOpenUser={onOpenUser} /><MaterialEngagement kind="publisher_news" materialId={item.id} ownerId={item.ownerId} currentUser={currentUser} users={users.length ? users : owner ? [owner] : []} onOpenUser={onOpenUser} shareAttachment={{ kind: "publisher_news", id: item.id }} />
  </section></div>;
}
...
export function ReadingModal({ item, currentUser, users = [], catalog: canonicalCatalog = [], likedUserIds = [], saved = false, savesCount = 0, onToggleLike, onToggleSave, onComment, onClose, onOpenUser, relationship, isFollowing, onAddFriend, onFollow, onEdit, onDelete, onReport }: { item: ReadingItem; currentUser?: DemoUser; users?: DemoUser[]; catalog?: (LibraryBook | AuthorBook)[]; likedUserIds?: number[]; saved?: boolean; savesCount?: number; onToggleLike?: () => void; onToggleSave?: () => void; onComment?: (text: string) => Promise<MaterialComment | null>; onClose: () => void; onOpenUser?: (userId: number) => void; relationship?: SocialRelationship; isFollowing?: boolean; onAddFriend?: (message: string) => void; onFollow?: () => void; onEdit?: () => void; onDelete?: () => void; onReport?: () => void }) {
  ...
  useEffect(() => {
    ...
    ]).then(([reactionData]) => { if (active) setHydratedLikedIds(reactionData.userIds ?? []); }).catch((error) => console.warn(error));
    void refresh();
  ...
  useEffect(() => {
    let active = true;
    apiFetch("/api/material-stats", { credentials: "same-origin", cache: "no-store" }).then((response) => response.json() as Promise<{ repostCounts?: Record<string, number> }>).then((data) => { if (active) setRepostCount(data.repostCounts?.[`${item.kind}-${item.id}`] ?? 0); }).catch(() => { if (active) setRepostCount(null); });
    return () => { active = false; };
  }, [item.id, item.kind]);
  ...
  return (
    <div className="modal-backdrop entity-page-backdrop" role="presentation" onMouseDown={routedClose}>
      <article className="reading-modal" role="dialog" aria-modal="true" aria-labelledby={item.kind === "review" ? "reading-title" : undefined} aria-label={item.kind === "excerpt" ? t("material.typePublication") : item.kind === "publisher_news" ? t("content.publisherNews") : undefined} onMouseDown={(event) => event.stopPropagation()}>
        <ModalIconActions leading={<>{currentUser && item.ownerId && currentUser.id !== item.ownerId && <HideUserAction userId={item.ownerId} icon onHidden={routedClose} />}{currentUser && (item.kind === "review" || item.kind === "excerpt") && repostCount !== null && <RepostAction kind={item.kind} materialId={item.id} repostCount={repostCount} />}</>} onEdit={onEdit} onDelete={onDelete} onReport={onReport} onClose={routedClose} />
        ...
        <SocialComments kind={item.kind} materialId={item.id} currentUser={currentUser} users={users} onOpenUser={onOpenUser} onCountChange={setCommentCount} />
...
export function UserProfileModal({ user, viewer, users, catalog, profileFriends = [], profileFollowers = [], profileCommunities = [], events = [], occasions = [], likes, friendCount, followerCount, relationship, incomingMessage, isFollowing, canMessage, blockedByMe = false, onClose, onAddFriend, onCancelFriendRequest, onAccept, onReject, onRemoveFriend, onOpenChat, onFollow, onUnfollow, onBlock, onUnblock, onReport, onToggleLike, onComment, onOpenUser }: { user: DemoUser; viewer: DemoUser; users: DemoUser[]; catalog: (LibraryBook | AuthorBook)[]; profileFriends?: DemoUser[]; profileFollowers?: DemoUser[]; profileCommunities?: DemoUser[]; events?: BookEvent[]; occasions?: Occasion[]; likes: Record<string, number[]>; friendCount?: number; followerCount?: number; relationship: SocialRelationship; incomingMessage?: string; isFollowing: boolean; canMessage: boolean; blockedByMe?: boolean; onClose: () => void; onAddFriend: (message: string) => void; onCancelFriendRequest: () => Promise<void>; onAccept: () => void; onReject: (comment: string) => void; onRemoveFriend: () => void; onOpenChat: () => void; onFollow: () => void; onUnfollow: () => Promise<void>; onBlock?: () => Promise<void>; onUnblock?: () => Promise<void>; onReport?: () => void; onToggleLike: (item: ReadingItem) => void; onComment: (item: ReadingItem, text: string) => Promise<MaterialComment | null>; onOpenUser: (userId: number) => void }) {
  ...
  const [hiddenByMe, setHiddenByMe] = useState(Boolean(user.hiddenByMe));
  ...
  const publicReadingMaterials: ReadingItem[] = [
    ...user.reviews.map((review) => ({ id: review.id, kind: "review" as const, title: review.bookTitle, author: user.profile.name, text: review.fullText, preview: review.preview, ownerId: user.id, createdAt: review.createdAt, bookAuthor: review.bookAuthor, rating: review.rating, isAdult: review.isAdult, mentions: review.mentions })),
    ...(user.excerpts ?? []).map((excerpt) => ({ id: excerpt.id, kind: "excerpt" as const, title: excerpt.bookTitle || t("content.publications"), author: user.profile.name, text: excerpt.text, preview: excerpt.previewText, bodyHtml: excerpt.bodyHtml, linkedBookId: excerpt.bookId, linkedBookIds: excerpt.bookIds, ownerId: user.id, createdAt: excerpt.createdAt, isAdult: excerpt.isAdult, mentions: excerpt.mentions })),
  ];
  const publicProfileMaterials = [
    ...
    ...(user.cleanReposts ?? []).map((item) => ({ type: "repost" as const, id: item.id, createdAt: item.createdAt, searchable: item.source.available ? item.source.title : "", item })),
    ...
    <div className="modal-backdrop profile-overlay-top" role="presentation" onMouseDown={routedClose}>
      <section className="public-profile-modal" role="dialog" aria-modal="true" aria-labelledby="public-profile-title" onMouseDown={(event) => event.stopPropagation()}>
        <div className="modal-icon-actions public-profile-icon-actions">
          ...
          {user.id !== viewer.id && <button className="modal-tool-button modal-report-button public-profile-block-button" type="button" aria-label={hiddenByMe ? "Вернуть материалы пользователя" : "Скрыть материалы пользователя"} data-tooltip={hiddenByMe ? "Вернуть материалы пользователя" : "Скрыть материалы пользователя"} onClick={async () => { if (!hiddenByMe && !window.confirm("Скрыть материалы пользователя?")) return; const response = await apiFetch(`/api/users/${user.id}/hide`, { method: hiddenByMe ? "DELETE" : "POST", credentials: "same-origin" }); if (response.ok) { setHiddenByMe(!hiddenByMe); window.dispatchEvent(new Event("bookmeet:bootstrap-refresh")); } }}><span aria-hidden="true">{hiddenByMe ? "↶" : "◉"}</span></button>}
...
type MentionInsertUser = { id: number; username: string; displayName: string };

function MentionInsertControl({ onInsert }: { onInsert: (user: MentionInsertUser) => void }) {
  const { t } = useI18n();
  const [query, setQuery] = useState("");
  const [options, setOptions] = useState<MentionInsertUser[]>([]);
  useEffect(() => {
    const normalized = query.trim().replace(/^@/, "").slice(0, 40);
    if (!normalized) { setOptions([]); return; }
    const controller = new AbortController();
    const timer = window.setTimeout(() => {
      apiFetch(`/api/users/mentions?q=${encodeURIComponent(normalized)}`, { credentials: "same-origin", signal: controller.signal })
        .then((response) => response.ok ? response.json() as Promise<{ users?: MentionInsertUser[] }> : { users: [] })
        .then((data) => setOptions(data.users ?? []))
        .catch((error) => { if (!(error instanceof Error && error.name === "AbortError")) setOptions([]); });
    }, 150);
    return () => { window.clearTimeout(timer); controller.abort(); };
  }, [query]);
  return <div className="mention-insert-control"><label><span>{t("mention.insert")}</span><input type="search" value={query} maxLength={41} placeholder="@username" aria-label={t("mention.insert")} autoComplete="off" onChange={(event) => setQuery(event.target.value)} /></label>{options.length > 0 && <ul className="mention-suggestions" role="listbox">{options.map((option) => <li key={option.id} role="option"><button type="button" onMouseDown={(event) => event.preventDefault()} onClick={() => { onInsert(option); setQuery(""); setOptions([]); }}>{option.displayName} <span>@{option.username}</span></button></li>)}</ul>}</div>;
}

function appendRichMention(html: string, user: MentionInsertUser, inlineImageLabel: string) {
  const token = `@${user.username}`.replace(/[&<>"']/g, (character) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" }[character] ?? character));
  return sanitizeRichHtml(`${html}${html.trim() ? "" : ""}<p>${token} </p>`, inlineImageLabel);
}
...
export function ReviewEditor({ review, catalog, onClose, onSave }: { review?: UserReview | null; catalog: (LibraryBook | AuthorBook)[]; onClose: () => void; onSave: (review: UserReview) => void | Promise<void> }) {
  ...
  const [form, setForm] = useState({ bookId: matchedInitialBook?.id, bookTitle: review?.bookTitle ?? "", bookAuthor: review?.bookAuthor ?? "", rating: review?.rating ?? 0, preview: review?.preview ?? "", fullText: review?.fullText ?? "", bodyHtml: review?.bodyHtml ?? (review?.fullText ? `<p>${review.fullText}</p>` : ""), mentions: review?.mentions ?? [], isAdult: review?.isAdult ?? false });
  const [ratingError, setRatingError] = useState(false);
  const [saving, setSaving] = useState(false);
  const [saveError, setSaveError] = useState("");
    ...
    <div className="modal-backdrop workflow-page-backdrop" role="presentation" onMouseDown={onClose}>
      <section className="review-editor blog-editor-modal" role="dialog" aria-modal="true" aria-labelledby="review-editor-title" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <form className="book-fields" onSubmit={submit}>
          ...
          <label className="review-preview-field">{t("review.preview")} *<span className="review-preview-shell"><MentionTextarea required rows={4} maxLength={500} placeholder={t("review.previewPlaceholder")} value={form.preview} onChange={(event) => setForm({ ...form, preview: event.target.value })} mentionUserIds={(form.mentions ?? []).map((item) => item.userId)} onMentionUserIdsChange={() => undefined} mentionRefs={form.mentions} onMentionRefsChange={(mentions) => setForm({ ...form, mentions })} /><small className={Array.from(form.preview).length >= 500 ? "limit-reached" : ""}>{Array.from(form.preview).length}/500</small></span></label>
          <div className="blog-composer"><span className="field-label blog-composer-title">{t("review.full")} *</span><div className="blog-text-block blog-rich-block"><span className="blog-block-title">{t("review.editorHint")}</span><MentionInsertControl onInsert={(user) => setForm((current) => ({ ...current, bodyHtml: appendRichMention(current.bodyHtml, user, t("editor.inlineImage")), mentions: [...(current.mentions ?? []).filter((mention) => mention.userId !== user.id), { userId: user.id, token: `@${user.username}`, username: user.username, displayName: user.displayName }] }))} /><RichTextEditor value={form.bodyHtml} onChange={(bodyHtml) => setForm({ ...form, bodyHtml })} catalog={catalog} /></div></div>
...
export function PublisherNewsEditor({ item, catalog = [], communityMode = false, onClose, onSave }: { item: PublisherNews; catalog?: (LibraryBook | AuthorBook)[]; communityMode?: boolean; onClose: () => void; onSave: (item: PublisherNews) => void | Promise<void> }) {
  ...
  return <div className="modal-backdrop workflow-page-backdrop" onMouseDown={() => { if (!saving) onClose(); }}><section className="review-editor blog-editor-modal publisher-news-editor" onMouseDown={(event) => event.stopPropagation()}><ResponsiveModalCloseButton onClose={onClose} /><h2>{item.id ? t("news.edit") : t("news.add")}</h2><form className="book-fields" onSubmit={submit}>
    <label>{t("content.title")}<MentionTextarea required rows={2} value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} mentionUserIds={(form.mentions ?? []).map((mention) => mention.userId)} onMentionUserIdsChange={() => undefined} mentionRefs={form.mentions} onMentionRefsChange={(mentions) => setForm((current) => ({ ...current, mentions }))} /></label>
    <div className="blog-composer"><span className="field-label blog-composer-title">{t("news.whatsNew")}</span><label className="blog-text-block blog-preview-field"><span className="blog-block-title">{t("news.previewHint")}</span><MentionTextarea required rows={7} maxLength={500} placeholder={t("news.previewPlaceholder")} value={form.previewText} onChange={(event) => setForm({ ...form, previewText: event.target.value })} mentionUserIds={(form.mentions ?? []).map((mention) => mention.userId)} onMentionUserIdsChange={() => undefined} mentionRefs={form.mentions} onMentionRefsChange={(mentions) => setForm((current) => ({ ...current, mentions }))} /><small className={form.previewText.length >= 500 ? "limit-reached" : ""}>{form.previewText.length}/500</small></label><div className="blog-text-block blog-rich-block"><span className="blog-block-title">{t("news.bodyHint")}</span><MentionInsertControl onInsert={(user) => setForm((current) => ({ ...current, bodyHtml: appendRichMention(current.bodyHtml ?? "", user, t("editor.inlineImage")), mentions: [...(current.mentions ?? []).filter((mention) => mention.userId !== user.id), { userId: user.id, token: `@${user.username}`, username: user.username, displayName: user.displayName }] }))} /><RichTextEditor value={form.bodyHtml ?? ""} onChange={(bodyHtml) => setForm({ ...form, bodyHtml })} catalog={catalog} /></div></div>
...
export function PublicationEditor({ excerpt, catalog: _catalog, onClose, onSave }: { excerpt?: UserExcerpt | null; catalog: (LibraryBook | AuthorBook)[]; onClose: () => void; onSave: (excerpt: UserExcerpt) => void | Promise<void> }) {
  ...
  return <div className="modal-backdrop workflow-page-backdrop" onMouseDown={onClose}><section className="review-editor publication-editor-simple" role="dialog" aria-modal="true" aria-labelledby="publication-editor-title" onMouseDown={(event) => event.stopPropagation()}><ResponsiveModalCloseButton onClose={onClose} /><h2 id="publication-editor-title">{excerpt ? t("publication.edit") : t("publication.add")}</h2><form className="book-fields" onSubmit={async (event) => { event.preventDefault(); const text = publicationText.trim(); if (!text || Array.from(text).length > 500) return; setSaving(true); setSaveError(""); try { await onSave({ ...form, bookId: undefined, bookIds: [], bookTitle: "", previewText: text, bodyHtml: `<p>${text}</p>`, text }); } catch (error) { setSaveError(error instanceof Error ? error.message : t("common.saveChangesError")); } finally { setSaving(false); } }}>
    <label className="publication-text-field"><span>{t("content.publications")}</span><MentionTextarea required autoFocus rows={9} maxLength={500} value={publicationText} onChange={(event) => setForm({ ...form, previewText: event.target.value, text: event.target.value })} placeholder={t("publication.placeholder")} mentionUserIds={(form.mentions ?? []).map((item) => item.userId)} onMentionUserIdsChange={() => undefined} mentionRefs={form.mentions} onMentionRefsChange={(mentions) => setForm({ ...form, mentions })} /><small className={publicationText.length >= 500 ? "limit-reached" : ""}>{publicationText.length}/500</small></label>
    {saveError && <p className="form-error" role="alert">{saveError}</p>}<div className="form-actions"><button type="button" disabled={saving} onClick={onClose}>{t("common.cancel")}</button><button className="primary-button" disabled={saving} type="submit">{saving ? t("auth.saving") : t("common.save")}</button></div>
  </form></section></div>;
}
...
export function AdminCatalogOverlay({ item, users, catalog = [], onClose, onEdit, onDelete }: { item: AdminCatalogItem; users: DemoUser[]; catalog?: (LibraryBook | AuthorBook)[]; onClose: () => void; onEdit: () => void; onDelete: () => void }) {
  ...
  const readingItem: ReadingItem = item.kind === "review"
    ? { id: item.id, kind: "review", title: (source as UserReview).bookTitle, bookAuthor: (source as UserReview).bookAuthor, author: source.ownerName, ownerId: source.ownerId, text: (source as UserReview).fullText, bodyHtml: (source as UserReview).bodyHtml, linkedBookId: (source as UserReview).bookId, preview: (source as UserReview).preview, rating: (source as UserReview).rating, createdAt: (source as UserReview).createdAt, isAdult: (source as UserReview).isAdult, mentions: (source as UserReview).mentions }
    : { id: item.id, kind: "excerpt", title: (source as UserExcerpt).bookTitle || t("content.publications"), linkedBookId: (source as UserExcerpt).bookId, linkedBookIds: (source as UserExcerpt).bookIds, author: source.ownerName, ownerId: source.ownerId, text: (source as UserExcerpt).text, preview: (source as UserExcerpt).previewText, bodyHtml: (source as UserExcerpt).bodyHtml, createdAt: (source as UserExcerpt).createdAt, isAdult: (source as UserExcerpt).isAdult, mentions: (source as UserExcerpt).mentions };
