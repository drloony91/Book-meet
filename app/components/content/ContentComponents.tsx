      ...
      <section className="public-profile-modal" role="dialog" aria-modal="true" aria-labelledby="public-profile-title" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="public-profile-body">
          ...
          <nav className="public-profile-tabs" aria-label={t("nav.profile")}>
            ...
            {tabVisible("publisher-news") && isOrganization && Boolean(user.publisherNews?.length) && <button className={activeTab === "publisher-news" ? "active" : ""} type="button" onClick={() => setActiveTab("publisher-news")}>{t(isCommunity ? "profile.communityNews" : "profile.publisherNews")} <span>{user.publisherNews?.length ?? 0}</span></button>}
