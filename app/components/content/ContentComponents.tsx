...
export function PublicationEditor({ excerpt, catalog, onClose, onSave }: { excerpt?: UserExcerpt | null; catalog: (LibraryBook | AuthorBook)[]; onClose: () => void; onSave: (excerpt: UserExcerpt) => void }) {
  const linkedBook = excerpt?.bookId ? catalog.find((book) => book.id === excerpt.bookId) : undefined;
  ...
  const [bookQuery, setBookQuery] = useState(linkedBook ? `${linkedBook.title} — ${linkedBook.author}` : "");
  const normalizedQuery = bookQuery.trim().toLowerCase();
  const matches = normalizedQuery.length >= 2 ? catalog.filter((book) => `${book.title} ${book.author}`.toLowerCase().includes(normalizedQuery)).slice(0, 6) : [];
  function selectBook(book: LibraryBook | AuthorBook) {
    setBookQuery(`${book.title} — ${book.author}`);
    setForm((current) => ({ ...current, bookId: book.id, bookTitle: book.title, link: "" }));
  }
  return <div className="modal-backdrop" onMouseDown={onClose}><section className="review-editor blog-editor-modal" onMouseDown={(event) => event.stopPropagation()}><button className="modal-close" type="button" onClick={onClose}>×</button><h2>{excerpt ? "Редактировать публикацию" : "Добавить публикацию"}</h2><form className="book-fields" onSubmit={(event) => { event.preventDefault(); onSave({ ...form, bookId: form.bookId, bookTitle: form.bookId ? form.bookTitle : "", previewText: form.previewText.trim(), bodyHtml: sanitizeRichHtml(form.bodyHtml ?? ""), text: form.previewText.trim() }); }}>
    <div className="material-book-search"><label>Название книги или автор<input value={bookQuery} placeholder="Начните вводить название книги или автора" onChange={(event) => { setBookQuery(event.target.value); setForm((current) => ({ ...current, bookId: undefined, bookTitle: "" })); }} /></label>{matches.length > 0 && <div className="book-match-suggestions">{matches.map((book) => <button type="button" key={`${book.id}-${book.title}`} onClick={() => selectBook(book)}><div className={`match-book-cover library-cover-${book.coverTone}`} style={book.coverUrl ? { backgroundImage: `url(${book.coverUrl})` } : undefined}>{!book.coverUrl && <span>{book.title.slice(0, 1)}</span>}</div><span><b>{book.title}</b><small>{book.author}</small></span></button>)}</div>}{form.bookId && <button className="selected-material-book" type="button" onClick={() => { setBookQuery(""); setForm((current) => ({ ...current, bookId: undefined, bookTitle: "" })); }}>Книга: {form.bookTitle} <span>×</span></button>}</div>
