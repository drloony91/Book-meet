...
export function MaterialActionBar({ likesCount = 0, commentsCount = 0, savesCount = 0, liked = false, saved = false, onToggleLike, onOpenComments, onToggleSave, shareAttachment }: MaterialActionProps) {
  ...
  return <div className="material-action-bar" aria-label={t("material.actions")}>
    ...
    {shareAttachment && <button type="button" aria-label={t("share.action")} title={t("share.action")} onClick={action(() => window.dispatchEvent(new CustomEvent("bookmeet:share-material", { detail: { attachment: shareAttachment } })))}><span aria-hidden="true">✈</span></button>}
  </div>;
}
      ...
      <section className="unified-book-modal" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="unified-book-layout">
          ...
          <div className="unified-book-copy">
            ...
            <button className="book-share-action" type="button" aria-label={t("share.action")} onClick={() => window.dispatchEvent(new CustomEvent("bookmeet:share-material", { detail: { attachment: { kind: "book", id: catalogBookId } } }))}>✈ {t("share.action")}</button><nav className="book-detail-tabs">
...
export function ReadingStatsModal({ books, users, viewer, onClose }: { books: LibraryBook[]; users: DemoUser[]; viewer?: DemoUser; onClose: () => void }) {
  const { t } = useI18n();
  ...
  return <div className="nested-modal-backdrop" onMouseDown={onClose}><section className="reading-stats-modal" onMouseDown={(event) => event.stopPropagation()}>
    ...
    {openedBook && <UnifiedBookModal book={openedBook} viewer={viewer} users={users} nested onClose={() => setOpenedBook(null)} />}
  </section></div>;
}
...
export function LibraryTab({ books, setBooks, userId, users, catalog = [], initialAdd = false, initialEditId }: { books: LibraryBook[]; setBooks: React.Dispatch<React.SetStateAction<LibraryBook[]>>; userId: number; users: DemoUser[]; catalog?: (LibraryBook | AuthorBook)[]; initialAdd?: boolean; initialEditId?: number | null }) {
  ...
  async function saveBook(book: LibraryBook) {
    ...
    try {
      const editingOwnedBook = books.some((item) => item.id === book.id);
      let response = await apiFetch(editingOwnedBook ? `/api/books/${book.id}` : "/api/books", { method: editingOwnedBook ? "PATCH" : "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(payload) });
  ...
  return (
    <div className="library-tab">
      <div className="profile-title-row library-title-row">
        <div><h1>{t("profile.library")}</h1><div className="library-status-summary" aria-label={t("library.readingStats")}><span>{t("library.wantSummary", { count: wantCount, books: booksWord(wantCount) })}</span><i aria-hidden="true" /><span>{t("library.readingSummary", { count: readingCount, books: booksWord(readingCount) })}</span><i aria-hidden="true" /><span>{t("library.readSummary", { count: books.filter((book) => (book.readingStatus ?? "read") === "read").length, books: booksWord(books.filter((book) => (book.readingStatus ?? "read") === "read").length) })}</span></div><div className="library-stats-actions"><button className="outline-button" type="button" onClick={() => setStatsOpen(true)}>{t("library.openStats")}</button><button className="outline-button" type="button" disabled aria-disabled="true">{t("library.goals")}</button></div></div>
      ...
      {statsOpen && <ReadingStatsModal books={books} users={users} viewer={viewer} onClose={() => setStatsOpen(false)} />}
    </div>
  );
}
