...
export function MaterialActionBar({ likesCount = 0, commentsCount = 0, savesCount = 0, liked = false, saved = false, onToggleLike, onOpenComments, onToggleSave }: MaterialActionProps) {
  ...
  return <div className="material-action-bar" aria-label={t("material.actions")}>
    ...
    <button type="button" aria-label={t("content.comments")} title={t("content.comments")} onClick={action(onOpenComments)}><img className="desktop-material-action-icon" src="/desktop-icons/comment.png" alt="" aria-hidden="true" /><span className="mobile-material-action-icon" aria-hidden="true">◯</span><b>{Math.max(0, commentsCount)}</b></button>
      ...
      <section className="public-profile-modal" role="dialog" aria-modal="true" aria-labelledby="public-profile-title" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="public-profile-body">
          ...
          <nav className="public-profile-tabs" aria-label={t("nav.profile")}>
            ...
            {!isOrganization && <button className={activeTab === "communities" ? "active" : ""} type="button" onClick={() => setActiveTab("communities")}>{t("profile.communities")} <span>{profileCommunities.length}</span></button>}
