...
export function EventForm({ initial, catalog = [], onCreateBook = () => undefined, onCancel, onSave, submitLabel = "Отправить на модерацию" }: { initial?: BookEvent; catalog?: (LibraryBook | AuthorBook)[]; onCreateBook?: () => void; onCancel: () => void; onSave: (value: EventFormValue) => Promise<void>; submitLabel?: string }) {
  ...
  return <form className="event-form" onSubmit={async (event) => { event.preventDefault(); setSaving(true); try { await onSave(value); } finally { setSaving(false); } }}>
    <button className="modal-close" type="button" aria-label="Закрыть" onClick={onCancel}>×</button>
    ...
    <div className="form-row"><CityAutocomplete value={value.city} required includeSpecialLocations onChange={(city, cityId) => setValue((current) => ({ ...current, city, cityId }))} /><label>Адрес<input required={!isSpecialLocation(value.city)} {...field("address")} /></label></div>
...
export function CityAutocomplete({ value, onChange, required = false, label = "Город", invalid = false, includeSpecialLocations = true }: { value: string; onChange: (name: string, id?: number, country?: string) => void; required?: boolean; label?: string; invalid?: boolean; includeSpecialLocations?: boolean }) {
  ...
  useEffect(() => {
    ...
    const controller = new AbortController();
    const timer = window.setTimeout(() => fetch(`/api/cities?q=${encodeURIComponent(query.trim())}`, { signal: controller.signal }).then((response) => response.json()).then((data: { cities?: CityOption[] }) => { setOptions(includeSpecialLocations ? [...SPECIAL_LOCATIONS.filter((name) => name.toLocaleLowerCase("ru").includes(query.trim().toLocaleLowerCase("ru"))).map((name) => ({ id: -1, name, countryCode: "", country: "Особое местоположение" })), ...(data.cities ?? [])] : data.cities ?? []); setOpen(true); }).catch(() => undefined), 180);
    return () => { window.clearTimeout(timer); controller.abort(); };
  }, [open, query, includeSpecialLocations]);
...
export function MaterialPreviewCard({ item, index = 0, owner, book, likesCount, commentsCount, kindLabel, onOpen, onOpenUser, onOpenAuthor, onOpenBook }: { item: ReadingItem; index?: number; owner?: Pick<DemoUser, "avatarUrl">; book?: LibraryBook | AuthorBook; likesCount?: number; commentsCount?: number; kindLabel?: string; onOpen: () => void; onOpenUser: (id: number) => void; onOpenAuthor?: () => void; onOpenBook?: () => void }) {
  ...
  return <article className={`excerpt-card material-preview-card material-clickable-card ${review ? "review-preview-card" : ""}`} role="button" tabIndex={0} onClick={onOpen} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onOpen(); } }}><span className="quote-mark">“</span><p>{item.preview || item.text}</p><div className="excerpt-footer"><div className={`author-dot author-dot-${index % 3 + 1} ${owner?.avatarUrl ? "has-photo" : ""}`} style={owner?.avatarUrl ? { backgroundImage: `url(${owner.avatarUrl})` } : undefined}>{!owner?.avatarUrl && item.author.slice(0, 1)}</div><div><h3>{kindLabel ?? (review ? <>Рецензия{item.rating ? ` · ★ ${item.rating}` : ""}</> : "Публикация")}</h3><span><button className="inline-user-link" type="button" onClick={(event) => { event.stopPropagation(); if (onOpenAuthor) onOpenAuthor(); else if (item.ownerId) onOpenUser(item.ownerId); }}>{item.author}</button>{book && <> · <button className="inline-book-link" type="button" onClick={(event) => { event.stopPropagation(); onOpenBook?.(); }}>{book.title}</button></>}</span>{(likesCount !== undefined || commentsCount !== undefined) && <small>♡ {likesCount ?? 0} · Комментарии: {commentsCount ?? 0}</small>}</div></div></article>;
}
...
export function PublisherNewsTab({ news, setNews, owner, users = [owner], catalog = [], canCreate = true, communityMode = false }: {
  ...
  return <div className="publisher-news-tab">
    ...
    {opened && <PublisherNewsModal item={opened} owner={owner} currentUser={owner} users={users} catalog={catalog} onOpenUser={() => undefined} onEdit={canCreate ? () => { begin(opened); setOpened(null); } : undefined} onDelete={canCreate ? () => { if (window.confirm(`Удалить новость «${opened.title}»?`)) { setNews((current) => current.filter((item) => item.id !== opened.id)); setOpened(null); } } : undefined} onClose={() => setOpened(null)} />}
