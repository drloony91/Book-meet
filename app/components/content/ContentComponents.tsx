...
export function OccasionForm({ initial, catalog = [], onCancel, onSave, submitLabel = "Отправить" }: { initial?: Occasion; catalog?: (LibraryBook | AuthorBook)[]; onCancel: () => void; onSave: (value: typeof emptyOccasion) => Promise<void>; submitLabel?: string }) {
  ...
  const targetMissing = value.type !== "invite" && !value.targetCities.length;
  ...
  return <form className="occasion-form" onSubmit={async (event) => { event.preventDefault(); if (!value.type || incompleteTime || targetMissing || locationMissing || bookMissing) return; setSaving(true); try { await onSave({ ...value, targetCities: value.type === "invite" ? [value.meetingCity] : value.targetCities }); } finally { setSaving(false); } }}>
    ...
    {value.type && texts && <>
      ...
      {value.type === "discuss" && <fieldset className="material-books-field required-book-field"><legend>Книга для обсуждения *</legend><EventBookSelector catalog={catalog} selectedId={value.linkedBookId} onSelect={(book) => setValue({ ...value, linkedBookId: book.id })} onClear={() => setValue({ ...value, linkedBookId: undefined })} onCreateBook={() => undefined} /></fieldset>}
...
export function OccasionCard({ item, own, onOpen, onEdit }: { item: Occasion; own: boolean; onOpen: () => void; onEdit?: () => void }) {
  ...
  return <article className="occasion-card material-clickable-card" role="button" tabIndex={0} onClick={onOpen} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onOpen(); } }}><div><span className="section-subtitle">{occasionLabels[item.type]}{schedule && <><i className="occasion-blue-dot" aria-hidden="true" />{schedule}</>}{item.isAdult ? " · 18+" : ""}</span>{own && <EventStatusLabel status={item.status} />}</div><h3>{labels.primary}: {item.primaryText}</h3><p><strong>{labels.audience}:</strong> {item.audienceText}</p><small>{item.type === "invite" ? [item.meetingCity, item.meetingAddress].filter(Boolean).join(" · ") : item.targetCities.join(" · ")}</small>{own && item.status === "needs_changes" && onEdit && <button className="outline-button card-inline-action" type="button" onClick={(event) => { event.stopPropagation(); onEdit(); }}>Редактировать</button>}</article>;
}
...
export function RichTextEditor({ value, onChange, catalog = [] }: { value: string; onChange: (value: string) => void; catalog?: (LibraryBook | AuthorBook)[] }) {
  ...
  function insertBook(book: LibraryBook | AuthorBook) {
    insertHtml(`<div class="rich-inline-book" data-book-id="${book.id}" contenteditable="false"><span class="rich-inline-book-cover" contenteditable="false">${book.title.slice(0, 1)}</span><span class="rich-inline-book-copy" contenteditable="false"><strong>${book.title.replace(/[<>&]/g, "")}</strong><small>${book.author.replace(/[<>&]/g, "")}</small></span><span class="rich-inline-book-remove" contenteditable="false">×</span></div><p><br></p>`);
  ...
  return <div className="rich-editor-shell">
    <div className="rich-editor-toolbar" aria-label="Форматирование текста">
      ...
      <div className="rich-book-tool"><button type="button" title="Вставить книгу" onMouseDown={(event) => { event.preventDefault(); rememberRange(); }} onClick={() => setBookSearchOpen((open) => !open)}>▣</button>{bookSearchOpen && <div className="rich-book-search"><input autoFocus value={bookQuery} onChange={(event) => setBookQuery(event.target.value)} placeholder="Начните вводить название книги или автора" />{bookMatches.map((book) => <button type="button" key={book.id} onClick={() => insertBook(book)}><strong>{book.title}</strong><small>{book.author}</small></button>)}{bookQuery.trim().length >= 2 && !bookMatches.length && <span>Книга не найдена</span>}</div>}</div>
