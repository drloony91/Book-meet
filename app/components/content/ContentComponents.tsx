...
type MaterialActionProps = { likesCount?: number; commentsCount?: number; savesCount?: number; liked?: boolean; saved?: boolean; onToggleLike?: () => void; onOpenComments?: () => void; onToggleSave?: () => void };
...
export function MaterialActionBar({ likesCount = 0, commentsCount = 0, savesCount = 0, liked = false, saved = false, onToggleLike, onOpenComments, onToggleSave }: MaterialActionProps) {
  ...
  return <div className="material-action-bar" aria-label={t("material.actions")}>
    <button type="button" className={liked ? "is-active" : ""} aria-pressed={liked} aria-label={t("material.like")} title={t("material.like")} onClick={action(onToggleLike)}><span aria-hidden="true">{liked ? "♥" : "♡"}</span><b>{Math.max(0, likesCount)}</b></button>
    <button type="button" aria-label={t("content.comments")} title={t("content.comments")} onClick={action(onOpenComments)}><span aria-hidden="true">◯</span><b>{Math.max(0, commentsCount)}</b></button>
    <button type="button" className={saved ? "is-active" : ""} aria-pressed={saved} aria-label={t("material.save")} title={t("material.save")} onClick={action(onToggleSave)}><span aria-hidden="true">{saved ? "▰" : "▱"}</span><b>{Math.max(0, savesCount)}</b></button>
  </div>;
}
...
export function OccasionCard({ item, own, owner, onOpen, onOpenUser, onEdit, ...actions }: { item: Occasion; own: boolean; owner?: DemoUser; onOpen: () => void; onOpenUser?: (id: number) => void; onEdit?: () => void } & MaterialActionProps) {
  ...
  return <article className="occasion-card material-clickable-card" role="button" tabIndex={0} onClick={onOpen} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onOpen(); } }}><header className="material-author-row"><div data-i18n-skip className={`author-dot ${owner?.avatarUrl ? "has-photo" : ""}`} style={owner?.avatarUrl ? { backgroundImage: `url(${owner.avatarUrl})` } : undefined}>{!owner?.avatarUrl && (owner?.initials ?? item.creatorName.slice(0, 2))}</div><div><button className="inline-user-link" data-i18n-skip type="button" onClick={(event) => { event.stopPropagation(); onOpenUser?.(item.creatorId); }}>{item.creatorName}{owner?.username ? ` (@${owner.username})` : ""}</button><small>{t("nav.dating")} · {item.createdAt}</small></div></header><div><span className="section-subtitle">{t(`occasion.${item.type}`)}{schedule && <><i className="occasion-blue-dot" aria-hidden="true" />{schedule}</>}{item.isAdult ? " · 18+" : ""}</span>{own && <EventStatusLabel status={item.status} />}</div><p className="occasion-audience-row"><strong>{t(labels.primary)}:</strong> <span data-i18n-skip>{item.primaryText}</span></p><p className="occasion-audience-row"><strong>{t(labels.audience)}:</strong> <span data-i18n-skip>{item.audienceText}</span></p><small data-i18n-skip>{item.type === "invite" ? [item.meetingCity, item.meetingAddress].filter(Boolean).join(" · ") : item.targetCities.join(" · ")}</small>{own && item.status === "needs_changes" && onEdit && <button className="outline-button card-inline-action" type="button" onClick={(event) => { event.stopPropagation(); onEdit(); }}>{t("common.edit")}</button>}<MaterialActionBar {...actions} onOpenComments={actions.onOpenComments ?? onOpen} /></article>;
}
...
export function OccasionModal({ item, currentUser, users = [], onClose, onOpenUser, onOpenBook, onEdit, onDelete, onReport }: { item: Occasion; currentUser?: DemoUser; users?: DemoUser[]; onClose: () => void; onOpenUser?: (id: number) => void; onOpenBook?: (id: number) => void; onEdit?: () => void; onDelete?: () => void; onReport?: () => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={routedClose}><section className="event-modal occasion-modal" onMouseDown={(event) => event.stopPropagation()}>
    <ModalIconActions onEdit={onEdit} onDelete={onDelete} onReport={onReport} onClose={routedClose} />
    {(() => { const owner = users.find((user) => user.id === item.creatorId); return <header className="material-author-row"><div data-i18n-skip className={`author-dot ${owner?.avatarUrl ? "has-photo" : ""}`} style={owner?.avatarUrl ? { backgroundImage: `url(${owner.avatarUrl})` } : undefined}>{!owner?.avatarUrl && (owner?.initials ?? item.creatorName.slice(0, 2))}</div><div><button className="inline-user-link" data-i18n-skip type="button" onClick={() => onOpenUser?.(item.creatorId)}>{item.creatorName}{owner?.username ? ` (@${owner.username})` : ""}</button><small data-i18n-skip>{item.createdAt}</small></div></header>; })()}
    <EventStatusLabel status={item.status} />
...
export function MaterialPreviewCard({ item, index = 0, owner, book, kindLabel, onOpen, onOpenUser, onOpenAuthor, onOpenBook, ...actions }: { item: ReadingItem; index?: number; owner?: { avatarUrl?: string; initials?: string; username?: string; profile?: { name: string } }; book?: LibraryBook | AuthorBook; kindLabel?: string; onOpen: () => void; onOpenUser: (id: number) => void; onOpenAuthor?: () => void; onOpenBook?: () => void } & MaterialActionProps) {
  const { t } = useI18n();
  const review = item.kind === "review";
  const username = owner?.username ? `@${owner.username}` : "";
  return <article className={`excerpt-card material-preview-card material-clickable-card ${review ? "review-preview-card" : ""}`} role="button" tabIndex={0} onClick={onOpen} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onOpen(); } }}><header className="material-author-row"><div data-i18n-skip className={`author-dot author-dot-${index % 3 + 1} ${owner?.avatarUrl ? "has-photo" : ""}`} style={owner?.avatarUrl ? { backgroundImage: `url(${owner.avatarUrl})` } : undefined}>{!owner?.avatarUrl && (owner?.initials || item.author.slice(0, 1))}</div><div><button className="inline-user-link" data-i18n-skip type="button" onClick={(event) => { event.stopPropagation(); if (onOpenAuthor) onOpenAuthor(); else if (item.ownerId) onOpenUser(item.ownerId); }}>{item.author}{username ? ` (${username})` : ""}</button><small>{kindLabel ?? (review ? <>{t("content.reviews")}{item.rating ? ` · ★ ${item.rating}` : ""}</> : t("content.publications"))}{item.createdAt ? ` · ${item.createdAt}` : ""}</small></div></header><p data-i18n-skip>{item.preview || item.text}</p>{book && <button className="inline-book-link" data-i18n-skip type="button" onClick={(event) => { event.stopPropagation(); onOpenBook?.(); }}>{book.title}</button>}<MaterialActionBar {...actions} onOpenComments={actions.onOpenComments ?? onOpen} /></article>;
}
...
export function PublisherNewsCard({ item, owner, onOpen, onOpenUser, ...actions }: { item: PublisherNews; owner?: DemoUser; onOpen: () => void; onOpenUser: (id: number) => void } & MaterialActionProps) {
  ...
  return <article className="excerpt-card material-preview-card publisher-news-card material-clickable-card" role="button" tabIndex={0} onClick={onOpen} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onOpen(); } }}><header className="material-author-row"><div data-i18n-skip className={`author-dot ${owner?.avatarUrl ? "has-photo" : ""}`} style={owner?.avatarUrl ? { backgroundImage: `url(${owner.avatarUrl})` } : undefined}>{!owner?.avatarUrl && (owner?.initials ?? "BM")}</div><div><button className="inline-user-link" data-i18n-skip type="button" onClick={(event) => { event.stopPropagation(); onOpenUser(item.ownerId); }}>{owner?.profile.name ?? domainLabel(community ? "Сообщество" : "Издатель")}{owner?.username ? ` (@${owner.username})` : ""}</button><small>{t("content.publisherNews")}{item.isAdult ? " · 18+" : ""} · {item.createdAt}</small></div></header><h3 data-i18n-skip>{item.title}</h3><p data-i18n-skip>{item.previewText}</p><MaterialActionBar {...actions} onOpenComments={actions.onOpenComments ?? onOpen} /></article>;
}
...
function PublicProfileDetails({ user }: { user: DemoUser }) {
  ...
  return <div className="public-profile-details">
    ...
    <div><span>{t("profile.gender")}</span><p>{domainLabel(user.profile.gender)}</p></div>
    ...
    <div className="modal-backdrop profile-overlay-top" role="presentation" onMouseDown={routedClose}>
      <section className="public-profile-modal" role="dialog" aria-modal="true" aria-labelledby="public-profile-title" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="public-profile-body">
          ...
          <h2 id="public-profile-title" data-i18n-skip style={{ fontSize: `${Math.max(16, 30 - Math.max(0, user.profile.name.length - 18) * .5)}px` }}>{user.profile.name}</h2><p className="public-profile-username" data-i18n-skip style={{ fontSize: `${Math.max(10, 14 - Math.max(0, user.username.length - 20) * .25)}px` }}>@{user.username}</p>{user.profile.city && <p className="profile-location" data-i18n-skip>⌖ {user.profile.city}</p>}<p className={`profile-presence ${user.online ? "is-online" : ""}`}>{user.online ? t("chat.online") : t("chat.offline")}</p>
...
export function PublicationEditor({ excerpt, catalog: _catalog, onClose, onSave }: { excerpt?: UserExcerpt | null; catalog: (LibraryBook | AuthorBook)[]; onClose: () => void; onSave: (excerpt: UserExcerpt) => void }) {
  const { t } = useI18n();
