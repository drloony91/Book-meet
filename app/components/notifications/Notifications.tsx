...
export function NotificationsPage({ notifications, users, onOpen, onMarkAllRead }: { notifications: SocialNotification[]; users: DemoUser[]; onOpen: (notification: SocialNotification) => void; onMarkAllRead: () => void }) {
  ...
  return <main className="notifications-page"><header><h1>{t("notifications.center")}</h1><button className="mark-read-button" type="button" onClick={onMarkAllRead}>{t("notifications.allRead")}</button></header><label className="notification-category-filter"><span>{t("notifications.category")}</span><CustomSelect ariaLabel={t("notifications.category")} value={tab} onChange={setTab} options={notificationCategoryKeys.map((item) => ({ value: item, label: labels[item] }))} /></label><div className="notifications-list">{visible.length ? visible.map((notification) => { const actor = users.find((user) => user.id === notification.actorId); return <button type="button" className={`notification-item ${notification.unread ? "unread" : ""}`} key={notification.id} onClick={() => onOpen(notification)}><span className={`notification-avatar avatar-${actor?.color ?? "navy"}`}>{actor?.initials ?? "BM"}</span><span><strong>{localizedNotificationTitle(locale, notification.type, notification.title)}</strong><p>{localizedNotificationText(locale, notification.type, notification.text, { name: actor?.profile.name })}</p><small>{localizedNotificationDate(locale, notification.createdAt)}</small></span></button>; }) : <div className="notifications-empty">{tab === "all" ? t("notifications.empty") : t("notifications.emptyCategory")}</div>}</div></main>;
}
