...
export function AdminSafetySection({ mode, reports, users, onBack, onOpenUser, onOpenChat, onRefresh }: {
  ...
  return <div className="admin-safety-page">
    ...
    {selected && <div className="modal-backdrop" onMouseDown={() => setSelected(null)}><section className="admin-report-modal" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <span className="section-subtitle" data-i18n-skip>{selected.reference ?? t("admin.reportNumber", { id: selected.id })}</span><h2>{selected.targetKind === "user" ? t("safety.user") : selected.targetKind === "chat" ? t("safety.chat") : selected.targetKind === "comment" ? t("safety.comment") : t("safety.material")}</h2>
      ...
      {["new", "reviewing"].includes(selected.status) && <><label>{t("safety.motivatedResponse")}<textarea required rows={5} value={motivatedResponse} onChange={(event) => setMotivatedResponse(event.target.value)} /></label><div className="admin-event-actions"><button className="outline-button" type="button" onClick={() => selected.targetUserId && onOpenChat(selected.targetUserId)}>{t("admin.messageUser")}</button>{selected.targetKind === "user" ? <button className="danger-button" type="button" onClick={() => setSuspending(true)}>{t("admin.block")}</button> : !["chat", "comment"].includes(selected.targetKind) ? <button className="danger-button" type="button" onClick={() => setDeleting(true)}>{t("admin.deleteMaterial")}</button> : null}{selected.status === "new" && <button className="outline-button" type="button" onClick={() => void action(`/api/admin/reports/${selected.id}`, { method: "PATCH", body: JSON.stringify({ status: "reviewing" }) })}>{t("safety.takeReview")}</button>}<button className="primary-button" disabled={!motivatedResponse.trim()} type="button" onClick={() => void action(`/api/admin/reports/${selected.id}`, { method: "PATCH", body: JSON.stringify({ status: "satisfied", response: motivatedResponse }) })}>{t("safety.satisfy")}</button><button className="outline-button" disabled={!motivatedResponse.trim()} type="button" onClick={() => void action(`/api/admin/reports/${selected.id}`, { method: "PATCH", body: JSON.stringify({ status: "rejected", response: motivatedResponse }) })}>{t("safety.reject")}</button></div></>}
    </section></div>}
