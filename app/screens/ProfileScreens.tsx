...
export function MyProfile({ onBack, user, users, friends, friendRequests, follows, events, occasions, likes, initialAction, initialEditId, initialEditing = false, onProfileCompleted, onToggleLike, onComment, onEditEvent, onDeleteEvent, onEditOccasion, onDeleteOccasion, onModerateEvent, onModerateOccasion, onLogout, onUserChange, onHomeViewChange, onOpenUser, onOpenChat }: { onBack: () => void; user: DemoUser; users: DemoUser[]; friends: DemoUser[]; friendRequests: FriendRequest[]; follows: Follow[]; events: BookEvent[]; occasions: Occasion[]; likes: Record<string, number[]>; initialAction?: "review" | "excerpt" | "book" | null; initialEditId?: number | null; initialEditing?: boolean; onProfileCompleted?: () => void; onToggleLike: (item: ReadingItem) => void; onComment: (item: ReadingItem, text: string) => Promise<MaterialComment | null>; onEditEvent: (item: BookEvent) => void; onDeleteEvent: (id: number) => void; onEditOccasion: (item: Occasion) => void; onDeleteOccasion: (id: number) => void; onModerateEvent: (id: number, action: "accept" | "revision" | "reject" | "edit", note?: string, event?: typeof emptyEvent, pinned?: boolean) => Promise<void>; onModerateOccasion: (id: number, action: "accept" | "revision" | "reject" | "edit", note?: string, occasion?: typeof emptyOccasion) => Promise<void>; onLogout: () => void; onUserChange: (user: DemoUser) => Promise<void>; onHomeViewChange: (homeView: "classic" | "feed") => Promise<void>; onOpenUser: (userId: number) => void; onOpenChat: (userId: number) => void }) {
  ...
  const profileTabs = ([
    ...
    { key: "friends", label: profile.type === "Сообщество" ? `Участники сообщества · ${friends.length}` : `Мои друзья · ${friends.length}` },
    ...
    <main className="my-profile-page">
      ...
      <section className="my-profile-card">
        ...
        <div className={`my-profile-main ${activeTab === "library" ? "library-main" : ""}`}>
            ...
            {editing ? (
              <form className="profile-form" noValidate onSubmit={save}>
                <div className="form-row">
                  <label className={invalidFields.name ? "field-invalid" : ""}>{profile.type === "Издатель" ? "Название издательства *" : profile.type === "Сообщество" ? "Название сообщества *" : "Имя *"}<input required aria-invalid={invalidFields.name} value={profile.name} onChange={(event) => { setProfile({ ...profile, name: event.target.value }); setInvalidFields((current) => ({ ...current, name: false })); }} /></label>
                ...
                {["Издатель", "Сообщество"].includes(profile.type) ? <>
                  <label>Ссылка на сайт {profile.type === "Сообщество" ? "сообщества" : "издательства"} *<input required type="url" value={profile.publisherWebsite ?? ""} onChange={(event) => setProfile({ ...profile, publisherWebsite: event.target.value })} /></label>
                  <label>О {profile.type === "Сообщество" ? "сообществе" : "издательстве"} *<textarea required rows={5} value={profile.bio} onChange={(event) => setProfile({ ...profile, bio: event.target.value })} /></label>
              ...
              <div className="profile-details">
                {["Издатель", "Сообщество"].includes(profile.type) ? <>
                  <div className="publisher-status-card"><span>Статус профиля</span><p>{profile.publisherStatus === "approved" ? `${profile.type === "Сообщество" ? "Сообщество" : "Издательство"} подтверждено` : profile.publisherStatus === "needs_changes" ? "Требуется доработка" : profile.publisherStatus === "rejected" ? "Профиль отклонён" : "Профиль находится на модерации"}</p>{profile.publisherModerationNote && <small>{profile.publisherModerationNote}</small>}</div>
                  <div className="profile-wide-field"><span>{profile.type === "Сообщество" ? "О сообществе" : "Об издательстве"}</span><p>{profile.bio}</p></div>
          ...
          {activeTab === "friends" && <ProfileFriendsTab friends={friends} outgoing={friendRequests.filter((request) => request.status === "pending" && request.fromId === user.id).map((request) => users.find((item) => item.id === request.toId)).filter(Boolean) as DemoUser[]} incoming={friendRequests.filter((request) => request.status === "pending" && request.toId === user.id).map((request) => users.find((item) => item.id === request.fromId)).filter(Boolean) as DemoUser[]} subscriptions={follows.filter((follow) => follow.followerId === user.id).map((follow) => users.find((item) => item.id === follow.targetId)).filter((item): item is DemoUser => Boolean(item) && !item!.isAdmin)} followers={follows.filter((follow) => follow.targetId === user.id).map((follow) => users.find((item) => item.id === follow.followerId)).filter((item): item is DemoUser => Boolean(item) && !item!.isAdmin)} communityMode={profile.type === "Сообщество"} onOpenUser={onOpenUser} />}
