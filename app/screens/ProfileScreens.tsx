...
function LinkedProfileControls({ profileType, settings = false }: { profileType: string; settings?: boolean }) {
  ...
  const [email, setEmail] = useState(""); const [password, setPassword] = useState(""); const [name, setName] = useState("Новое сообщество"); const [notice, setNotice] = useState(""); const [busy, setBusy] = useState(false);
  ...
  return <div className={settings ? "linked-profile-settings" : "linked-profile-actions"}>{settings ? <><h2>Привязанный профиль</h2><p>Свяжите личный профиль с одним профилем сообщества.</p></> : <button className="outline-button" type="button" onClick={() => setMode("choose")}>Привязать сообщество</button>}{settings && <button className="outline-button linked-profile-settings-button" type="button" onClick={() => setMode("choose")}>Привязать сообщество</button>}{mode !== "none" && <div className="notice-backdrop" onMouseDown={() => setMode("none")}><section className="confirm-social-modal linked-profile-modal" onMouseDown={(event) => event.stopPropagation()}><button className="modal-close" type="button" aria-label="Закрыть" onClick={() => setMode("none")}>×</button>{mode === "choose" ? <div className="linked-profile-choice"><h2>Привязать сообщество</h2><p>Вы можете создать новое сообщество или привязать уже существующий профиль.</p><div className="form-actions"><button className="outline-button" type="button" onClick={() => setMode("create")}>Создать новое</button><button className="primary-button" type="button" onClick={() => setMode("attach")}>Привязать существующее</button></div></div> : <div className="auth-form-page linked-profile-auth"><h2>{mode === "create" ? "Новое сообщество" : "Вход в профиль сообщества"}</h2><form onSubmit={submit}>{mode === "create" && <label>Название сообщества<input required autoComplete="organization" value={name} onChange={(event) => setName(event.target.value)} placeholder="Например: Книжный клуб" /></label>}<label>E-mail<input required type="email" autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="you@example.com" /></label><label>Пароль<input required type="password" minLength={8} autoComplete={mode === "create" ? "new-password" : "current-password"} value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Не менее 8 знаков" /></label>{notice && <span className="login-error">{notice}</span>}<button className="primary-button" disabled={busy} type="submit">{busy ? "Проверяем…" : mode === "create" ? "Зарегистрировать" : "Войти"}</button></form><div className="auth-provider-actions"><button className="google-provider-button linked-google-button" type="button" disabled={busy} onClick={() => void googleLink()}>{mode === "create" ? "Зарегистрировать через Google" : "Войти через Google"}</button></div><button className="outline-button linked-profile-back" type="button" onClick={() => setMode("choose")}>Назад</button></div>}</section></div>}</div>;
}
