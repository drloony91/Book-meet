    ...
    <main className="my-profile-page">
      ...
      <section className="my-profile-card">
        ...
        <div className={`my-profile-main ${activeTab === "library" ? "library-main" : ""}`}>
            ...
            {editing ? (
              <form className="profile-form" noValidate onSubmit={save}>
                <div className="form-row">
                  ...
                  <CityAutocomplete value={profile.city} required invalid={invalidFields.city} onChange={(city, cityId) => { setProfile({ ...profile, city, cityId }); setInvalidFields((current) => ({ ...current, city: false })); }} />
                </div>
          ...
          {activeTab === "events" && <MyEventsTab createdEvents={events.filter((item) => item.creatorId === user.id)} participatingEvents={events.filter((item) => item.creatorId !== user.id && item.reminderSet)} users={users} onEdit={onEditEvent} onDeleted={onDeleteEvent} />}
