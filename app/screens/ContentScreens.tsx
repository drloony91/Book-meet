...
export function HomeContent({ reviews, excerpts, events, occasions, currentUserType, onOpenUser, onCreateEvent, onEditEvent, onCreateOccasion, onEditOccasion, onCreateReview, onCreateExcerpt, onNavigate, currentUserName, currentUser, users, relationshipFor, isFollowing, onAddFriend, onFollow, likes, onToggleLike, onComment }: { reviews: Review[]; excerpts: Excerpt[]; events: BookEvent[]; occasions: Occasion[]; currentUserType: "reader" | "writer" | "blogger"; onOpenUser: (userId: number) => void; onCreateEvent: () => void; onEditEvent: (item: BookEvent) => void; onCreateOccasion: () => void; onEditOccasion: (item: Occasion) => void; onCreateReview: () => void; onCreateExcerpt: () => void; onNavigate: (page: "reviews" | "publications" | "events" | "occasions") => void; currentUserName: string; currentUser: DemoUser; users: DemoUser[]; relationshipFor: (userId: number) => "none" | "outgoing" | "incoming" | "friends"; isFollowing: (userId: number) => boolean; onAddFriend: (userId: number, message: string) => void; onFollow: (userId: number) => void; likes: Record<string, number[]>; onToggleLike: (item: ReadingItem) => void; onComment: (item: ReadingItem, text: string) => Promise<MaterialComment | null> }) {
  ...
  return (
    <main className="content-scroll home-content">
      ...
      <section className="content-section">
        ...
        {reviews.length > 0 ? (
          <div className="excerpt-grid review-material-grid">
            {reviews.map((review, index) => { const reviewBook = catalogFromUsers(users).find((book) => book.title.toLowerCase() === review.book.replace(/[«»]/g, "").toLowerCase() && book.author.toLowerCase() === review.author.toLowerCase()); const item: ReadingItem = { id: review.id, kind: "review", title: review.book, author: review.user, text: review.fullText, ownerId: review.ownerId, createdAt: review.createdAt, preview: review.quote, bookAuthor: review.author }; return <MaterialPreviewCard key={`${review.ownerId}-${review.id}`} item={item} index={index} book={reviewBook} onOpen={() => setReadingItem(item)} onOpenUser={onOpenUser} onOpenBook={reviewBook ? () => setOpenedBlogBook(reviewBook) : undefined} />; })}
          </div>
        ) : (
        ...
        {excerpts.length > 0 ? (
          <div className="excerpt-grid">
            {excerpts.map((excerpt, index) => {
              ...
              return (
            <MaterialPreviewCard key={`${excerpt.ownerId}-${excerpt.id}`} item={{ id: excerpt.id, kind: "excerpt", title: linkedBook?.title || "Публикация", author: excerpt.author, text: excerpt.fullText, bodyHtml: excerpt.bodyHtml, linkedBookId: excerpt.linkedBookId, ownerId: excerpt.ownerId, createdAt: excerpt.createdAt, preview: excerpt.text }} index={index} book={linkedBook} onOpen={() => setReadingItem({ id: excerpt.id, kind: "excerpt", title: linkedBook?.title || "Публикация", author: excerpt.author, text: excerpt.fullText, bodyHtml: excerpt.bodyHtml, linkedBookId: excerpt.linkedBookId, ownerId: excerpt.ownerId, createdAt: excerpt.createdAt, preview: excerpt.text })} onOpenUser={onOpenUser} onOpenBook={linkedBook ? () => setOpenedBlogBook(linkedBook) : undefined} />
              );
            })}
          </div>
        ) : (
      ...
      {openedBlogBook && <UnifiedBookModal book={openedBlogBook} users={users} onClose={() => setOpenedBlogBook(null)} onReport={!currentUser.isAdmin && !currentUser.books.some((book) => book.id === openedBlogBook.id) && !(currentUser.authorBooks ?? []).some((book) => book.id === openedBlogBook.id) ? () => openReportDialog({ kind: "book", id: openedBlogBook.id }) : undefined} />}
      {openedEvent && <EventModal item={openedEvent} onOpenBook={openedEvent.linkedBookId ? () => {
      ...
      <section className="content-section occasions-section">
        <div className="section-heading"><div className="interactive-section-title"><h2>Поводы познакомиться</h2><HomeScopeSwitch city={currentCity} value={occasionScope} onChange={setOccasionScope} /></div><div className="section-heading-actions"><button type="button" className="secondary-action-button" onClick={onCreateOccasion}>Предложить повод</button><button type="button" className="secondary-action-button" onClick={() => onNavigate("occasions")}>Смотреть всё</button></div></div>
...
export function EventsDirectoryPage({ events, currentUser, users, onHome, onCreate, onEdit }: { events: BookEvent[]; currentUser: DemoUser; users: DemoUser[]; onHome: () => void; onCreate: () => void; onEdit: (item: BookEvent) => void }) {
