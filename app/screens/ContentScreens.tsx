  ...
  return (
    <main className={`content-scroll home-content home-mode-${homeMode}`}>
      {homeMode === "feed" && <section className="content-section home-feed">{feedEntries.map((entry, index) => {
        if (entry.kind === "event") { const reading = { id: entry.item.id, kind: "event", title: entry.item.title, author: users.find((user) => user.id === entry.item.creatorId)?.profile.name ?? t("material.user"), text: entry.item.description, preview: entry.item.summary, ownerId: entry.item.creatorId, createdAt: entry.item.createdAt } as ReadingItem; const key = `event-${entry.item.id}`; return <EventCard key={key} item={entry.item} own={entry.item.creatorId === currentUser.id} likesCount={likes[key]?.length} commentsCount={commentCounts[key]} savesCount={saveCounts[key]} liked={likes[key]?.includes(currentUser.id)} saved={saves[key]?.includes(currentUser.id)} onToggleLike={() => onToggleLike(reading)} onToggleSave={() => onToggleSave(reading)} onOpen={() => setOpenedEvent(entry.item)} onEdit={() => onEditEvent(entry.item)} />; }
        if (entry.kind === "occasion") { const reading = { id: entry.item.id, kind: "occasion", title: entry.item.primaryText, author: entry.item.creatorName, text: entry.item.audienceText, preview: entry.item.primaryText, ownerId: entry.item.creatorId, createdAt: entry.item.createdAt } as ReadingItem; const key = `occasion-${entry.item.id}`; return <OccasionCard key={key} item={entry.item} own={entry.item.creatorId === currentUser.id} likesCount={likes[key]?.length} commentsCount={commentCounts[key]} savesCount={saveCounts[key]} liked={likes[key]?.includes(currentUser.id)} saved={saves[key]?.includes(currentUser.id)} onToggleLike={() => onToggleLike(reading)} onToggleSave={() => onToggleSave(reading)} onOpen={() => setOpenedOccasion(entry.item)} onEdit={() => onEditOccasion(entry.item)} />; }
