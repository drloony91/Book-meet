import { BookShelfCard } from "../components/books/BookShelves";
...
import { includesFollowingFeed } from "../lib/feed-filter";
...
export function HomeContent({ reviews, excerpts, publisherNews, events, occasions, catalog = [], currentUserType, onOpenUser, onCreateEvent, onEditEvent, onCreateOccasion, onEditOccasion, onCreateReview, onCreateExcerpt, onNavigate, currentUserName, currentUser, users, relationshipFor, isFollowing, onAddFriend, onFollow, likes, saves, commentCounts, saveCounts, onToggleLike, onToggleSave, onComment }: { reviews: Review[]; excerpts: Excerpt[]; publisherNews: PublisherNews[]; events: BookEvent[]; occasions: Occasion[]; catalog?: (LibraryBook | AuthorBook)[]; currentUserType: "reader" | "writer" | "blogger"; onOpenUser: (userId: number) => void; onCreateEvent: () => void; onEditEvent: (item: BookEvent) => void; onCreateOccasion: () => void; onEditOccasion: (item: Occasion) => void; onCreateReview: () => void; onCreateExcerpt: () => void; onNavigate: (page: "reviews" | "publications" | "events" | "occasions") => void; currentUserName: string; currentUser: DemoUser; users: DemoUser[]; relationshipFor: (userId: number) => SocialRelationship; isFollowing: (userId: number) => boolean; onAddFriend: (userId: number, message: string) => void; onFollow: (userId: number) => void; likes: Record<string, number[]>; saves: Record<string, number[]>; commentCounts: Record<string, number>; saveCounts: Record<string, number>; onToggleLike: (item: ReadingItem) => void; onToggleSave: (item: ReadingItem) => void; onComment: (item: ReadingItem, text: string) => Promise<MaterialComment | null> }) {
  ...
  const visibleFeedEntries = feedEntries.filter((entry) => {
    const searchText = entry.kind === "event" ? `${entry.item.title} ${entry.item.summary} ${entry.item.description}` : entry.kind === "occasion" ? `${entry.item.primaryText} ${entry.item.audienceText} ${entry.item.creatorName}` : entry.kind === "review" ? `${entry.item.book} ${entry.item.author} ${entry.item.fullText} ${entry.item.quote}` : entry.kind === "excerpt" ? `${entry.item.author} ${entry.item.fullText} ${entry.item.text}` : `${entry.item.title} ${entry.item.previewText} ${entry.item.bodyHtml}`;
    return (feedTab === "all" || includesFollowingFeed(entry, currentUser.id, (id) => relationshipFor(id) === "friends", isFollowing)) && (!feedNeedle || searchText.toLocaleLowerCase("ru").includes(feedNeedle));
  });

  return (
    <main className={`content-scroll home-content home-mode-${homeMode}`}>
      {homeMode === "feed" && <section className="content-section home-feed"><div className={`feed-tabs home-scope-switch scope-${feedTab === "following" ? "following" : "all"}`} role="tablist" aria-label={t("feed.ariaLabel")}><span aria-hidden="true" /><button type="button" role="tab" aria-selected={feedTab === "all"} className={feedTab === "all" ? "active" : ""} onClick={() => setFeedTab("all")}>{t("feed.all")}</button><button type="button" role="tab" aria-selected={feedTab === "following"} className={feedTab === "following" ? "active" : ""} onClick={() => setFeedTab("following")}>{t("feed.following")}</button></div><label className="desktop-feed-search"><span aria-hidden="true">⌕</span><input type="search" value={feedQuery} onChange={(event) => setFeedQuery(event.target.value)} placeholder={t("desktop.search.feed")} /></label>{visibleFeedEntries.map((entry, index) => {
        if (entry.kind === "event") { const reading = { id: entry.item.id, kind: "event", title: entry.item.title, author: users.find((user) => user.id === entry.item.creatorId)?.profile.name ?? t("material.user"), text: entry.item.description, preview: entry.item.summary, ownerId: entry.item.creatorId, createdAt: entry.item.createdAt, mentions: entry.item.mentions } as ReadingItem; const key = `event-${entry.item.id}`; return <EventCard key={key} item={entry.item} owner={users.find((user) => user.id === entry.item.creatorId)} own={entry.item.creatorId === currentUser.id} likesCount={likes[key]?.length} commentsCount={commentCounts[key]} savesCount={saveCounts[key]} liked={likes[key]?.includes(currentUser.id)} saved={saves[key]?.includes(currentUser.id)} onToggleLike={() => onToggleLike(reading)} onToggleSave={() => onToggleSave(reading)} onOpen={() => setOpenedEvent(entry.item)} onOpenUser={onOpenUser} onEdit={() => onEditEvent(entry.item)} />; }
        if (entry.kind === "occasion") { const reading = { id: entry.item.id, kind: "occasion", title: entry.item.primaryText, author: entry.item.creatorName, text: entry.item.audienceText, preview: entry.item.primaryText, ownerId: entry.item.creatorId, createdAt: entry.item.createdAt, mentions: entry.item.mentions } as ReadingItem; const key = `occasion-${entry.item.id}`; return <OccasionCard key={key} item={entry.item} owner={users.find((user) => user.id === entry.item.creatorId)} own={entry.item.creatorId === currentUser.id} likesCount={likes[key]?.length} commentsCount={commentCounts[key]} savesCount={saveCounts[key]} liked={likes[key]?.includes(currentUser.id)} saved={saves[key]?.includes(currentUser.id)} onToggleLike={() => onToggleLike(reading)} onToggleSave={() => onToggleSave(reading)} onOpen={() => setOpenedOccasion(entry.item)} onOpenUser={onOpenUser} onEdit={() => onEditOccasion(entry.item)} />; }
        if (entry.kind === "review") { const review = entry.item; const book = resolvedCatalog.find((item) => item.id === review.linkedBookId) ?? resolvedCatalog.find((item) => item.title.toLowerCase() === review.book.replace(/[«»]/g, "").toLowerCase() && item.author.toLowerCase() === review.author.toLowerCase()); const reading: ReadingItem = { id: review.id, kind: "review", title: review.book, author: review.user, text: review.fullText, bodyHtml: review.bodyHtml, linkedBookId: review.linkedBookId, ownerId: review.ownerId, createdAt: review.createdAt, preview: review.quote, bookAuthor: review.author, rating: Number(review.rating), mentions: review.mentions }; const key = `review-${review.id}`; return <MaterialPreviewCard key={key} item={reading} index={index} owner={users.find((user) => user.id === review.ownerId)} book={book} likesCount={likes[key]?.length} commentsCount={commentCounts[key]} savesCount={saveCounts[key]} liked={likes[key]?.includes(currentUser.id)} saved={saves[key]?.includes(currentUser.id)} onToggleLike={() => onToggleLike(reading)} onToggleSave={() => onToggleSave(reading)} onOpen={() => setReadingItem(reading)} onOpenUser={onOpenUser} onOpenBook={book ? () => setOpenedBlogBook(book) : undefined} />; }
        ...
        const excerpt = entry.item; const book = excerpt.linkedBookId ? resolvedCatalog.find((item) => item.id === excerpt.linkedBookId) : undefined; const reading: ReadingItem = { id: excerpt.id, kind: "excerpt", title: book?.title || t("content.publications"), author: excerpt.author, text: excerpt.fullText, preview: excerpt.text, bodyHtml: excerpt.bodyHtml, linkedBookId: excerpt.linkedBookId, linkedBookIds: excerpt.linkedBookIds, ownerId: excerpt.ownerId, createdAt: excerpt.createdAt, mentions: excerpt.mentions }; const key = `excerpt-${excerpt.id}`; return <MaterialPreviewCard key={key} item={reading} index={index} owner={users.find((user) => user.id === excerpt.ownerId)} book={book} likesCount={likes[key]?.length} commentsCount={commentCounts[key]} savesCount={saveCounts[key]} liked={likes[key]?.includes(currentUser.id)} saved={saves[key]?.includes(currentUser.id)} onToggleLike={() => onToggleLike(reading)} onToggleSave={() => onToggleSave(reading)} onOpen={() => setReadingItem(reading)} onOpenUser={onOpenUser} onOpenBook={book ? () => setOpenedBlogBook(book) : undefined} />;
      })}{!visibleFeedEntries.length && (feedTab === "following" ? <div className="content-empty"><span aria-hidden="true">···</span><h3>{t("feed.followingEmpty")}</h3><p><a href="/users">{t("feed.findUsers")}</a> · <a href="/communities">{t("feed.findCommunities")}</a></p></div> : <EmptyContentState />)}</section>}
      ...
      <section className="content-section">
        ...
        {reviews.length > 0 ? (
          <div className="excerpt-grid review-material-grid">
            {reviews.map((review, index) => { const reviewBook = resolvedCatalog.find((book) => book.id === review.linkedBookId) ?? resolvedCatalog.find((book) => book.title.toLowerCase() === review.book.replace(/[«»]/g, "").toLowerCase() && book.author.toLowerCase() === review.author.toLowerCase()); const item: ReadingItem = { id: review.id, kind: "review", title: review.book, author: review.user, text: review.fullText, bodyHtml: review.bodyHtml, linkedBookId: review.linkedBookId, ownerId: review.ownerId, createdAt: review.createdAt, preview: review.quote, bookAuthor: review.author, rating: Number(review.rating), mentions: review.mentions }; return <MaterialPreviewCard key={`${review.ownerId}-${review.id}`} item={item} index={index} owner={users.find((user) => user.id === review.ownerId)} book={reviewBook} onOpen={() => setReadingItem(item)} onOpenUser={onOpenUser} onOpenBook={reviewBook ? () => setOpenedBlogBook(reviewBook) : undefined} />; })}
          </div>
        ) : (
        ...
        {excerpts.length > 0 ? (
          <div className="excerpt-grid">
            {excerpts.map((excerpt, index) => {
              ...
              return (
            <MaterialPreviewCard key={`${excerpt.ownerId}-${excerpt.id}`} item={{ id: excerpt.id, kind: "excerpt", title: linkedBook?.title || t("content.publications"), author: excerpt.author, text: excerpt.fullText, bodyHtml: excerpt.bodyHtml, linkedBookId: excerpt.linkedBookId, linkedBookIds: excerpt.linkedBookIds, ownerId: excerpt.ownerId, createdAt: excerpt.createdAt, preview: excerpt.text, mentions: excerpt.mentions }} index={index} owner={users.find((user) => user.id === excerpt.ownerId)} book={linkedBook} onOpen={() => setReadingItem({ id: excerpt.id, kind: "excerpt", title: linkedBook?.title || t("content.publications"), author: excerpt.author, text: excerpt.fullText, bodyHtml: excerpt.bodyHtml, linkedBookId: excerpt.linkedBookId, linkedBookIds: excerpt.linkedBookIds, ownerId: excerpt.ownerId, createdAt: excerpt.createdAt, preview: excerpt.text, mentions: excerpt.mentions })} onOpenUser={onOpenUser} onOpenBook={linkedBook ? () => setOpenedBlogBook(linkedBook) : undefined} />
              );
            })}
          </div>
        ) : (
      ...
      {openedBlogBook && <UnifiedBookModal book={openedBlogBook} viewer={currentUser} users={users} catalog={resolvedCatalog} onClose={() => setOpenedBlogBook(null)} onOpenUser={onOpenUser} onReport={!currentUser.isAdmin && !currentUser.books.some((book) => book.id === openedBlogBook.id) && !(currentUser.authorBooks ?? []).some((book) => book.id === openedBlogBook.id) ? () => openReportDialog({ kind: "book", id: openedBlogBook.id }) : undefined} />}
...
export function MaterialsDirectoryPage({ kind, reviews, excerpts, publisherNews = [], currentUser, users, catalog = [], likes, saves, commenters, commentCounts, saveCounts, onCreate, onToggleLike, onToggleSave, onComment, onOpenUser, relationshipFor, isFollowing, onAddFriend, onFollow }: { kind: "review" | "excerpt"; reviews: Review[]; excerpts: Excerpt[]; publisherNews?: PublisherNews[]; currentUser: DemoUser; users: DemoUser[]; catalog?: (LibraryBook | AuthorBook)[]; likes: Record<string, number[]>; saves: Record<string, number[]>; commenters: Record<string, number[]>; commentCounts: Record<string, number>; saveCounts: Record<string, number>; onCreate: () => void; onToggleLike: (item: ReadingItem) => void; onToggleSave: (item: ReadingItem) => void; onComment: (item: ReadingItem, text: string) => Promise<MaterialComment | null>; onOpenUser: (userId: number) => void; relationshipFor: (userId: number) => SocialRelationship; isFollowing: (userId: number) => boolean; onAddFriend: (userId: number, message: string) => void; onFollow: (userId: number) => void }) {
  ...
  return <main className="content-scroll directory-page">
    ...
    <div className="excerpt-grid directory-material-grid">
      {kind === "review" ? (sorted as Review[]).map((review, index) => { const matchingBook = resolvedCatalog.find((book) => book.id === review.linkedBookId) ?? resolvedCatalog.find((book) => book.title.toLowerCase() === review.book.replace(/[«»]/g, "").toLowerCase() && book.author.toLowerCase() === review.author.toLowerCase()); const item: ReadingItem = { id: review.id, kind: "review", title: review.book, author: review.user, text: review.fullText, bodyHtml: review.bodyHtml, linkedBookId: review.linkedBookId, ownerId: review.ownerId, createdAt: review.createdAt, preview: review.quote, bookAuthor: review.author, rating: Number(review.rating), mentions: review.mentions }; return <MaterialPreviewCard key={`${review.ownerId}-${review.id}`} item={item} index={index} owner={users.find((user) => user.id === review.ownerId)} book={matchingBook} likesCount={likes[`review-${review.id}`]?.length ?? 0} commentsCount={commenters[`review-${review.id}`]?.length ?? 0} onOpen={() => setOpened(item)} onOpenUser={onOpenUser} onOpenBook={matchingBook ? () => setOpenedBook(matchingBook) : undefined} />; }) : <>{(sorted as Excerpt[]).map((excerpt, index) => { const linkedBook = excerpt.linkedBookId ? resolvedCatalog.find((book) => book.id === excerpt.linkedBookId) : undefined; const item: ReadingItem = { id: excerpt.id, kind: "excerpt", title: linkedBook?.title || t("content.publications"), author: excerpt.author, text: excerpt.fullText, preview: excerpt.text, bodyHtml: excerpt.bodyHtml, linkedBookId: excerpt.linkedBookId, ownerId: excerpt.ownerId, createdAt: excerpt.createdAt, mentions: excerpt.mentions }; return <MaterialPreviewCard key={`${excerpt.ownerId}-${excerpt.id}`} item={item} index={index} owner={users.find((user) => user.id === excerpt.ownerId)} book={linkedBook} likesCount={likes[`excerpt-${excerpt.id}`]?.length ?? 0} commentsCount={commenters[`excerpt-${excerpt.id}`]?.length ?? 0} onOpen={() => setOpened(item)} onOpenUser={onOpenUser} onOpenBook={linkedBook ? () => setOpenedBook(linkedBook) : undefined} />; })}{sortedPublisherNews.map((item) => <PublisherNewsCard key={`publisher-news-${item.id}`} item={item} owner={users.find((user) => user.id === item.ownerId)} onOpen={() => setOpenedPublisherNews(item)} onOpenUser={onOpenUser} />)}</>}
    </div>
    {!sorted.length && (kind === "review" || !sortedPublisherNews.length) && <EmptyContentState />}
    {opened && <ReadingModal item={opened} currentUser={currentUser} users={users} catalog={resolvedCatalog} likedUserIds={likes[`${opened.kind}-${opened.id}`] ?? []} onToggleLike={() => onToggleLike(opened)} onComment={(text) => onComment(opened, text)} onOpenUser={onOpenUser} onClose={() => setOpened(null)} relationship={opened.ownerId ? relationshipFor(opened.ownerId) : "none"} isFollowing={opened.ownerId ? isFollowing(opened.ownerId) : false} onAddFriend={(message) => opened.ownerId && onAddFriend(opened.ownerId, message)} onFollow={() => opened.ownerId && onFollow(opened.ownerId)} onReport={!currentUser.isAdmin && opened.ownerId !== currentUser.id ? () => openReportDialog({ kind: opened.kind, id: opened.id }) : undefined} />}
    {openedPublisherNews && <PublisherNewsModal item={openedPublisherNews} owner={users.find((user) => user.id === openedPublisherNews.ownerId)} currentUser={currentUser} users={users} catalog={resolvedCatalog} onClose={() => setOpenedPublisherNews(null)} onOpenUser={onOpenUser} onReport={!currentUser.isAdmin ? () => openReportDialog({ kind: "publisher_news", id: openedPublisherNews.id }) : undefined} />}
    {openedBook && <UnifiedBookModal book={openedBook} viewer={currentUser} users={users} catalog={resolvedCatalog} onClose={() => setOpenedBook(null)} onReport={!currentUser.isAdmin && !currentUser.books.some((book) => book.id === openedBook.id) && !(currentUser.authorBooks ?? []).some((book) => book.id === openedBook.id) ? () => openReportDialog({ kind: "book", id: openedBook.id }) : undefined} />}
  </main>;
}
...
export function AllBooksDirectoryPage({ users = [], currentUser, onOpenUser = () => undefined, publicBooks, onBookOpen }: { users?: DemoUser[]; currentUser?: DemoUser; onOpenUser?: (id: number) => void; publicBooks?: PublicCatalogBook[]; onBookOpen?: (book: PublicCatalogBook) => void }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      setLoading(true);
      const suffix = normalized.length >= 2 ? `?q=${encodeURIComponent(normalized)}` : "";
      setLoadError("");
      apiFetch(`/api/books/catalog${suffix}`, { credentials: "same-origin", cache: "no-store" }).then(async (response) => { if (!response.ok) throw new Error("catalog"); return response.json() as Promise<{ books?: Array<Partial<CatalogDirectoryBook> & Pick<CatalogDirectoryBook, "id" | "author" | "title" | "addedAt" | "popularity">> }>; }).then((data) => { if (!active) return; setBooks((data.books ?? []).map(catalogDirectoryBook)); }).catch((error) => { if (active && (!(error instanceof Error) || error.name !== "AbortError")) setLoadError(t("directory.bookSearchError")); }).finally(() => { if (active) setLoading(false); });
    }, 280);
    return () => { active = false; window.clearTimeout(timer); };
  }, [publicBooks, query]);
  const visible = useMemo(() => books.slice().sort((first, second) => sort === "added" ? Date.parse(second.addedAt) - Date.parse(first.addedAt) || first.title.localeCompare(second.title, "ru") : sort === "popular" ? second.popularity - first.popularity || first.title.localeCompare(second.title, "ru") : first.title.localeCompare(second.title, "ru") || first.author.localeCompare(second.author, "ru")), [books, sort]);
  const openBook = (book: CatalogDirectoryBook) => onBookOpen ? onBookOpen(book) : setOpened(book);
  return <main className="content-scroll directory-page books-directory-page"><div className="directory-heading"><div><h1>{t("nav.books")}</h1><p>{t("directory.booksCount", { count: visible.length })}</p></div><div className="directory-controls books-directory-controls"><label className="books-search-field"><span>{t("directory.searchBooks")}</span><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("directory.bookSearchPlaceholder")} /></label><label className="directory-control-field"><span>{t("common.sort")}</span><CustomSelect ariaLabel={t("directory.sortBooks")} value={sort} onChange={setSort} options={[{ value: "alphabetical", label: t("directory.alphabetical") }, { value: "added", label: t("directory.dateAdded") }, { value: "popular", label: t("directory.popular") }]} /></label></div></div>{loadError && <p className="form-error" role="alert">{loadError}</p>}{loading ? <p className="directory-loading">{t("directory.loadingBooks")}</p> : visible.length ? <div className="all-books-grid">{visible.map((book) => <article className="library-book material-clickable-card" role="button" tabIndex={0} key={book.id} onClick={() => openBook(book)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); openBook(book); } }}><div className="all-books-cover-frame"><div className={`library-book-cover library-cover-${book.coverTone}`} data-i18n-skip style={book.coverUrl ? { backgroundImage: `url(${book.coverUrl})` } : undefined}>{!book.coverUrl && <><em>{book.author}</em><strong>{book.title}</strong><span>Book Meet</span></>}</div></div><div className="library-book-copy"><h3 data-i18n-skip>{book.title}</h3><p data-i18n-skip>{book.author}</p>{book.ratingCount ? <small aria-label={t("book.ratingAria", { title: book.title })}>★ {book.averageRating?.toLocaleString("ru-RU", { minimumFractionDigits: 1, maximumFractionDigits: 1 })} ({book.ratingCount})</small> : <small>{t("directory.inLibraries", { count: book.popularity })}</small>}</div></article>)}</div> : <EmptyContentState />}{opened && currentUser && <UnifiedBookModal book={opened} viewer={currentUser} users={users} catalog={books} onOpenUser={onOpenUser} onClose={() => setOpened(null)} onReport={!currentUser.isAdmin && opened.creatorUserId !== currentUser.id ? () => openReportDialog({ kind: "book", id: opened.id }) : undefined} />}</main>;
}
...
export function PersonalMaterialFeed({ mode, refs, shelves = [], users, events, occasions, publisherNews, currentUser, likes, saves, commentCounts, saveCounts, onToggleLike, onToggleSave, onOpenUser, onOpenMaterial }: { mode: "liked" | "saved"; refs: MaterialActionRef[]; shelves?: BookShelf[]; users: DemoUser[]; events: BookEvent[]; occasions: Occasion[]; publisherNews: PublisherNews[]; currentUser: DemoUser; likes: Record<string, number[]>; saves: Record<string, number[]>; commentCounts: Record<string, number>; saveCounts: Record<string, number>; onToggleLike: (item: ReadingItem) => void; onToggleSave: (item: ReadingItem) => void; onOpenUser: (id: number) => void; onOpenMaterial: (kind: MaterialActionRef["kind"], id: number) => void }) {
  ...
  for (const ref of refs) {
    if (ref.kind === "shelf") { const shelf = shelves.find((item) => item.id === ref.id); if (shelf) entries.push({ key: `shelf-${shelf.id}`, createdAt: ref.createdAt, node: <BookShelfCard shelf={shelf} /> }); continue; }
    if (ref.kind === "event") { const source = events.find((entry) => entry.id === ref.id); if (source) { const item: ReadingItem = { id: source.id, kind: "event", title: source.title, author: users.find((user) => user.id === source.creatorId)?.profile.name ?? t("material.user"), text: source.description, preview: source.summary, ownerId: source.creatorId, createdAt: source.createdAt, mentions: source.mentions }; entries.push({ key: `event-${source.id}`, createdAt: ref.createdAt, node: <EventCard item={source} own={source.creatorId === currentUser.id} {...actionProps(item)} onOpen={() => onOpenMaterial("event", source.id)} /> }); } continue; }
    if (ref.kind === "occasion") { const source = occasions.find((entry) => entry.id === ref.id); if (source) { const item: ReadingItem = { id: source.id, kind: "occasion", title: source.primaryText, author: source.creatorName, text: source.audienceText, preview: source.primaryText, ownerId: source.creatorId, createdAt: source.createdAt, mentions: source.mentions }; entries.push({ key: `occasion-${source.id}`, createdAt: ref.createdAt, node: <OccasionCard item={source} own={source.creatorId === currentUser.id} {...actionProps(item)} onOpen={() => onOpenMaterial("occasion", source.id)} /> }); } continue; }
    if (ref.kind === "publisher_news") { const source = publisherNews.find((entry) => entry.id === ref.id); if (source) { const owner = users.find((user) => user.id === source.ownerId); const item: ReadingItem = { id: source.id, kind: "publisher_news", title: source.title, author: owner?.profile.name ?? "Book Meet", text: source.body, preview: source.previewText, ownerId: source.ownerId, createdAt: source.createdAt, mentions: source.mentions }; entries.push({ key: `publisher-news-${source.id}`, createdAt: ref.createdAt, node: <PublisherNewsCard item={source} owner={owner} {...actionProps(item)} onOpen={() => onOpenMaterial("publisher_news", source.id)} onOpenUser={onOpenUser} /> }); } continue; }
