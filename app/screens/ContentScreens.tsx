    ...
    <main className={`content-scroll home-content home-mode-${homeMode}`}>
      ...
      <section className="content-section">
        ...
        {excerpts.length > 0 ? (
          ...
          <EmptyContentState action={currentUserType === "writer" || currentUserType === "blogger" ? t("directory.firstPublication") : undefined} onAction={onCreateExcerpt} />
        )}
      </section>
      ...
      {writerOnlyWarning && <div className="modal-backdrop" onMouseDown={() => setWriterOnlyWarning(false)}><section className="simple-warning-modal" role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}><h2>{t("directory.writerOnly")}</h2><button className="primary-button" type="button" onClick={() => setWriterOnlyWarning(false)}>{t("common.close")}</button></section></div>}
    </main>
  );
}
...
export function OccasionsDirectoryPage({ occasions, currentUser, users, catalog = [], onHome, onCreate, onEdit, onOpenUser }: { occasions: Occasion[]; currentUser: DemoUser; users: DemoUser[]; catalog?: (LibraryBook | AuthorBook)[]; onHome: () => void; onCreate: () => void; onEdit: (item: Occasion) => void; onOpenUser: (id: number) => void }) {
  ...
  const [openedBook, setOpenedBook] = useState<LibraryBook | AuthorBook | null>(null);
  const resolvedCatalog = catalog.length ? catalog : catalogFromUsers(users);
  const visible = occasions.filter((item) => item.status === "published" || item.creatorId === currentUser.id).filter((item) => locationMatchesCityFilter(item.targetCities, city)).filter((item) => item.creatorId === currentUser.id || (item.targetGender === "Все" || item.targetGender === currentUser.profile.gender) && (item.targetProfileType === "Все" || item.targetProfileType === currentUser.profile.type));
