...
test("community books, profile completion and avatar contracts are enforced in both adapters", async () => {
  const [m035, m036, api, demo, data, compliance, profiles, content, directory, header, css, index, controller, apiFetch, messages] = await Promise.all([
  ...
  assert.match(profiles, /CommunityBooksTab/); assert.match(profiles, /profile\.type === "Издатель" && profile\.publisherStatus === "approved"/);
  ...
  assert.match(content, /<legend>Книга месяца<\/legend>/);
  ...
  assert.match(directory, /pageSize = 12/); assert.match(directory, /\["Читатель", "Писатель", "Блогер"\]/);
  assert.match(header, /book-meet-brand-v4\.png/); assert.match(index, /book-meet-favicon-v3\.png/);
