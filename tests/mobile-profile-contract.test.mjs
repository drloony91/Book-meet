...
test("mobile profiles use routed full-page states without replacing desktop profile flows", async () => {
  ...
  assert.match(publicProfile, /mobile-public-profile-social-links"><button/);
  assert.match(publicProfile, /disabled=\{!user\.profile\.canViewFriends\}/);
  assert.match(publicProfile, /disabled=\{!user\.profile\.canViewFollowers\}/);
